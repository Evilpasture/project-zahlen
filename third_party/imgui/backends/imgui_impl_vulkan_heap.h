...
//     In order to make sure this is visible from the imgui_impl_vulkan_heap.cpp compilation unit:
//     - Add '#define IMGUI_IMPL_VULKAN_NO_PROTOTYPES' in your imconfig.h file
//     - Or as a compilation flag in your build system
//     - Or uncomment here (not recommended because you'd be modifying imgui sources!)
//     - Do not simply add it in a .cpp file!
// (2) Call ImGui_ImplVulkanHeap_LoadFunctions() before ImGui_ImplVulkanHeap_Init() with your custom function.
// If you have no idea what this is, leave it alone!
//#define IMGUI_IMPL_VULKAN_NO_PROTOTYPES

// [Configuration] Convenience support for Volk
// (you can also technically use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkanHeap_LoadFunctions().)
...
// Backend uses a small number of descriptors per font atlas + as many as additional calls done to ImGui_ImplVulkanHeap_AddTexture().
...
struct ImGui_ImplVulkanHeap_PipelineInfo
{
...
// Initialization data, for ImGui_ImplVulkanHeap_Init()
...
// VK_EXT_descriptor_heap configuration (Zahlen fork). See ImGui_ImplVulkanHeap_InitInfo::HeapInfo.
struct ImGui_ImplVulkanHeap_HeapInfo
{
    const void* HeapContext        = nullptr;  // const ZHLN::Vk::Context*
    void*       HeapManager        = nullptr;  // ZHLN::Vk::HeapManager*
    uint32_t    ResourceSlotBase   = 0;        // first texture slot (font = +0)
    uint32_t    ResourceSlotCount  = 0;        // 0 = legacy descriptor-set mode
    uint32_t    ResourceStride     = 0;        // bytes per resource descriptor
    uint32_t    SamplerSlotLinear  = 0;        // static sampler-heap slot
    uint32_t    SamplerSlotNearest = 0;        // static sampler-heap slot
    uint32_t    SamplerStride      = 0;        // bytes per sampler descriptor
    uint32_t    PushIndexOffset    = 16;       // push-data offset of the texture-index word (PC block = 16 bytes)
    uint32_t    SamplerPushOffset  = 20;       // push-data offset of the sampler-index word
};

struct ImGui_ImplVulkanHeap_InitInfo
{
    ...
    // Pipeline
    ImGui_ImplVulkanHeap_PipelineInfo   PipelineInfoMain;           // Infos for Main Viewport (created by app/user)
    ...
    // When HeapInfo.ResourceSlotCount > 0 the backend renders through the
    // engine's descriptor heaps instead of descriptor sets/pools. The
    // descriptor pool fields above are ignored. HeapContext/HeapManager point
    // at the engine's ZHLN::Vk::Context / ZHLN::Vk::HeapManager; the backend
    // sub-allocates ResourceSlotCount slots starting at ResourceSlotBase for
    // textures (slot 0 = font atlas) and writes its two sampler descriptors
    // into SamplerSlotLinear/SamplerSlotNearest.
    ImGui_ImplVulkanHeap_HeapInfo HeapInfo;
};

// Follow "Getting Started" link and check examples/ folder to learn about using backends!
IMGUI_IMPL_API bool             ImGui_ImplVulkanHeap_Init(ImGui_ImplVulkanHeap_InitInfo* info);
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_Shutdown();
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_NewFrame();
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline = VK_NULL_HANDLE);
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_SetMinImageCount(uint32_t min_image_count); // To override MinImageCount after initialization (e.g. if swap chain is recreated)

// (Advanced) Use e.g. if you need to recreate pipeline without reinitializing the backend (see #8110, #8111)
// The main window pipeline will be created by ImGui_ImplVulkanHeap_Init() if possible (== RenderPass xor (UseDynamicRendering && PipelineRenderingCreateInfo->sType == VK_STRUCTURE_TYPE_PIPELINE_RENDERING_CREATE_INFO_KHR))
// Else, the pipeline can be created, or re-created, using ImGui_ImplVulkanHeap_CreateMainPipeline() before rendering.
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_CreateMainPipeline(const ImGui_ImplVulkanHeap_PipelineInfo* info);

// (Advanced) Use e.g. if you need to precisely control the timing of texture updates (e.g. for staged rendering), by setting ImDrawData::Textures = nullptr to handle this manually.
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_UpdateTexture(ImTextureData* tex);

// Register a texture (VkDescriptorSet for a VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE == ImTextureID)
// VK_EXT_descriptor_heap mode: registers a heap slot and returns it as ImTextureID.
// image_view_info is REQUIRED in heap mode (heap image descriptors consume VkImageViewCreateInfo).
IMGUI_IMPL_API VkDescriptorSet  ImGui_ImplVulkanHeap_AddTexture(VkImageView image_view, VkImageLayout image_layout);
IMGUI_IMPL_API VkDescriptorSet  ImGui_ImplVulkanHeap_AddTextureEx(VkImageView image_view, VkImageLayout image_layout, const VkImageViewCreateInfo* image_view_info);
IMGUI_IMPL_API void             ImGui_ImplVulkanHeap_RemoveTexture(VkDescriptorSet descriptor_set);

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
IMGUI_IMPL_API VkDescriptorSet  ImGui_ImplVulkanHeap_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout); // Ignore VkSampler
#endif

// Optional: load Vulkan functions with a custom function loader
// This is only useful with IMGUI_IMPL_VULKAN_NO_PROTOTYPES / VK_NO_PROTOTYPES
IMGUI_IMPL_API bool             ImGui_ImplVulkanHeap_LoadFunctions(uint32_t api_version, PFN_vkVoidFunction(*loader_func)(const char* function_name, void* user_data), void* user_data = nullptr);

// [BETA] Selected render state data shared with callbacks.
// This is temporarily stored in GetPlatformIO().Renderer_RenderState during the ImGui_ImplVulkanHeap_RenderDrawData() call.
// (Please open an issue if you feel you need access to more data)
struct ImGui_ImplVulkanHeap_RenderState
{
...
// by the regular ImGui_ImplVulkanHeap_XXX functions.
//-------------------------------------------------------------------------
...
struct ImGui_ImplVulkanH_Window
{
    ...
    VkPipeline              Pipeline;           // The window pipeline may uses a different VkRenderPass than the one passed in ImGui_ImplVulkanHeap_InitInfo
