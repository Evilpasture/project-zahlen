...
// The aim of imgui_impl_vulkan_heap.h/.cpp is to be usable in your engine without any modification.
...
// Important note to the reader who wish to integrate imgui_impl_vulkan_heap.cpp/.h in their own engine/app.
// - Common ImGui_ImplVulkanHeap_XXX functions and structures are used to interface with imgui_impl_vulkan_heap.cpp/.h.
//   You will use those if you want to use this rendering backend in your engine/app.
// - Helper ImGui_ImplVulkanH_XXX functions and structures are only used by this example (main.cpp) and by
//   the backend itself (imgui_impl_vulkan_heap.cpp), but should PROBABLY NOT be used by your own engine/app code.
// Read comments in imgui_impl_vulkan_heap.h.
...
//              - When registering custom textures: changed ImGui_ImplVulkanHeap_AddTexture() signature to remove Sampler.
//                - Before: ImGui_ImplVulkanHeap_AddTexture(VkSampler, VkImageView, VkImageLayout)
//                - After:  ImGui_ImplVulkanHeap_AddTexture(VkImageView, VkImageLayout)
...
//  2026-03-11: Vulkan: Added ImGui_ImplVulkanHeap_PipelineInfo::ExtraDynamicStates[] to allow specifying extra dynamic states to add when creating the VkPipeline. (#9211)
...
//  2025-09-26: *BREAKING CHANGE*: moved some fields in ImGui_ImplVulkanHeap_InitInfo: init_info.RenderPass --> init_info.PipelineInfoMain.RenderPass, init_info.Subpass --> init_info.PipelineInfoMain.Subpass, init_info.MSAASamples --> init_info.PipelineInfoMain.MSAASamples, init_info.PipelineRenderingCreateInfo --> init_info.PipelineInfoMain.PipelineRenderingCreateInfo.
//  2025-09-26: *BREAKING CHANGE*: renamed ImGui_ImplVulkanHeap_MainPipelineCreateInfo to ImGui_ImplVulkanHeap_PipelineInfo. Introduced very recently so shouldn't affect many users.
...
//  2025-09-26: Vulkan: Added a way to customize shaders by filling ImGui_ImplVulkanHeap_InitInfo::CustomShaderVertCreateInfo/CustomShaderFragCreateInfo. (#8585)
//  2025-09-18: Call platform_io.ClearRendererHandlers() on shutdown.
//  2025-09-04: Vulkan: Added ImGui_ImplVulkanHeap_CreateMainPipeline(). (#8110, #8111)
...
//  2025-06-11: Vulkan: Added support for ImGuiBackendFlags_RendererHasTextures, for dynamic font atlas. Removed ImGui_ImplVulkanHeap_CreateFontsTexture() and ImGui_ImplVulkanHeap_DestroyFontsTexture().
...
//  2025-04-07: Vulkan: Deep-copy ImGui_ImplVulkanHeap_InitInfo::PipelineRenderingCreateInfo's pColorAttachmentFormats buffer when set, in order to reduce common user-error of specifying a pointer to data that gets out of scope. (#8282)
//  2025-02-14: *BREAKING CHANGE*: Added uint32_t api_version to ImGui_ImplVulkanHeap_LoadFunctions().
//  2025-02-13: Vulkan: Added ApiVersion field in ImGui_ImplVulkanHeap_InitInfo. Default to header version if unspecified. Dynamic rendering path loads "vkCmdBeginRendering/vkCmdEndRendering" (without -KHR suffix) on API 1.3. (#8326)
...
//  2024-10-07: Vulkan: Expose selected render state in ImGui_ImplVulkanHeap_RenderState, which you can access in 'void* platform_io.Renderer_RenderState' during draw callbacks.
...
//  2024-04-19: Vulkan: Added convenience support for Volk via IMGUI_IMPL_VULKAN_USE_VOLK define (you can also use IMGUI_IMPL_VULKAN_NO_PROTOTYPES + wrap Volk via ImGui_ImplVulkanHeap_LoadFunctions().)
//  2024-02-14: *BREAKING CHANGE*: Moved RenderPass parameter from ImGui_ImplVulkanHeap_Init() function to ImGui_ImplVulkanHeap_InitInfo structure. Not required when using dynamic rendering.
...
//  2024-01-03: Vulkan: Added MinAllocationSize field in ImGui_ImplVulkanHeap_InitInfo to workaround zealous "best practice" validation layer. (#7189, #4238)
...
//  2023-11-10: *BREAKING CHANGE*: Removed parameter from ImGui_ImplVulkanHeap_CreateFontsTexture(): backend now creates its own command-buffer to upload fonts.
//              *BREAKING CHANGE*: Removed ImGui_ImplVulkanHeap_DestroyFontUploadObjects() which is now unnecessary as we create and destroy those objects in the backend.
//              ImGui_ImplVulkanHeap_CreateFontsTexture() is automatically called by NewFrame() the first time.
//              You can call ImGui_ImplVulkanHeap_CreateFontsTexture() again to recreate the font atlas texture.
//              Added ImGui_ImplVulkanHeap_DestroyFontsTexture() but you probably never need to call this.
...
//  2023-01-02: Vulkan: Fixed sampler passed to ImGui_ImplVulkanHeap_AddTexture() not being honored + removed a bunch of duplicate code.
//  2022-10-11: Using 'nullptr' instead of 'NULL' as per our switch to C++11.
//  2022-10-04: Vulkan: Added experimental ImGui_ImplVulkanHeap_RemoveTexture() for api symmetry. (#914, #5738).
//  2022-01-20: Vulkan: Added support for ImTextureID as VkDescriptorSet. User need to call ImGui_ImplVulkanHeap_AddTexture(). Building for 32-bit targets requires '#define ImTextureID ImU64'. (#914).
...
//  2021-01-27: Vulkan: Added support for custom function load and IMGUI_IMPL_VULKAN_NO_PROTOTYPES by using ImGui_ImplVulkanHeap_LoadFunctions().
//  2020-11-11: Vulkan: Added support for specifying which subpass to reference during VkPipeline creation.
//  2020-09-07: Vulkan: Added VkPipeline parameter to ImGui_ImplVulkanHeap_RenderDrawData (default to one passed to ImGui_ImplVulkanHeap_Init).
//  2020-05-04: Vulkan: Fixed crash if initial frame has no vertices.
//  2020-04-26: Vulkan: Fixed edge case where render callbacks wouldn't be called if the ImDrawData didn't have vertices.
//  2019-08-01: Vulkan: Added support for specifying multisample count. Set ImGui_ImplVulkanHeap_InitInfo::MSAASamples to one of the VkSampleCountFlagBits values to use, default is non-multisampled as before.
//  2019-05-29: Vulkan: Added support for large mesh (64K+ vertices), enable ImGuiBackendFlags_RendererHasVtxOffset flag.
//  2019-04-30: Vulkan: Added support for special ImDrawCallback_ResetRenderState callback to reset render state.
//  2019-04-04: *BREAKING CHANGE*: Vulkan: Added ImageCount/MinImageCount fields in ImGui_ImplVulkanHeap_InitInfo, required for initialization (was previously a hard #define IMGUI_VK_QUEUED_FRAMES 2). Added ImGui_ImplVulkanHeap_SetMinImageCount().
...
//  2018-06-22: Inverted the parameters to ImGui_ImplVulkanHeap_RenderDrawData() to be consistent with other backends.
//  2018-06-08: Misc: Extracted imgui_impl_vulkan_heap.cpp/.h away from the old combined GLFW+Vulkan example.
...
//  2018-03-01: Vulkan: Renamed ImGui_ImplVulkanHeap_Init_Info to ImGui_ImplVulkanHeap_InitInfo and fields to match more closely Vulkan terminology.
//  2018-02-16: Misc: Obsoleted the io.RenderDrawListsFn callback, ImGui_ImplVulkanHeap_Render() calls ImGui_ImplVulkanHeap_RenderDrawData() itself.
...
// Zahlen descriptor-heap bridge: the fork renders through the engine's
// VK_EXT_descriptor_heap resource/sampler heaps instead of descriptor sets.
...
struct ImGui_ImplVulkanHeap_FrameRenderBuffers;
struct ImGui_ImplVulkanHeap_WindowRenderBuffers;
bool ImGui_ImplVulkanHeap_CreateDeviceObjects();
void ImGui_ImplVulkanHeap_DestroyDeviceObjects();
void ImGui_ImplVulkanHeap_DestroyFrameRenderBuffers(VkDevice device, ImGui_ImplVulkanHeap_FrameRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
void ImGui_ImplVulkanHeap_DestroyWindowRenderBuffers(VkDevice device, ImGui_ImplVulkanHeap_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator);
...
// (see description of IMGUI_IMPL_VULKAN_NO_PROTOTYPES in imgui_impl_vulkan_heap.h)
...
// Reusable buffers used for rendering 1 current in-flight frame, for ImGui_ImplVulkanHeap_RenderDrawData()
...
struct ImGui_ImplVulkanHeap_WindowRenderBuffers
{
    uint32_t            Index;
    uint32_t            Count;
    ImVector<ImGui_ImplVulkanHeap_FrameRenderBuffers> FrameRenderBuffers;
};

struct ImGui_ImplVulkanHeap_Texture
{
    VkDeviceMemory              Memory;
    VkImage                     Image;
    VkImageView                 ImageView;
    VkDescriptorSet             DescriptorSet;
    VkImageViewCreateInfo       ViewInfo; // VK_EXT_descriptor_heap: heap image descriptors consume create infos

    ImGui_ImplVulkanHeap_Texture() { memset((void*)this, 0, sizeof(*this)); }
};

// Vulkan data
struct ImGui_ImplVulkanHeap_Data
{
    ImGui_ImplVulkanHeap_InitInfo   VulkanInitInfo;
    ImGui_ImplVulkanHeap_RenderState* RenderState;          // == ImGui::GetPlatformIO().Renderer_RenderState during rendering.
    ...
    ImGui_ImplVulkanHeap_WindowRenderBuffers MainWindowRenderBuffers;

    // --- VK_EXT_descriptor_heap mode (Zahlen fork) ---
    bool                     HeapMode = false;
    uint32_t                 NextHeapTextureSlot = 1; // slot 0 = font atlas
    ImVector<uint32_t>       FreeHeapTextureSlots;
    uint32_t                 CurrentSamplerHeapIndex = 0; // 0 = linear, 1 = nearest
    const ZHLN::Vk::Context* HeapContext = nullptr;
    ZHLN::Vk::HeapManager*   HeapManager = nullptr;
    VkSamplerCreateInfo      SamplerLinearInfo {};
    VkSamplerCreateInfo      SamplerNearestInfo {};

    ImGui_ImplVulkanHeap_Data()
    {
...
static ImGui_ImplVulkanHeap_Data* ImGui_ImplVulkanHeap_GetBackendData()
{
    return ImGui::GetCurrentContext() ? (ImGui_ImplVulkanHeap_Data*)ImGui::GetIO().BackendRendererUserData : nullptr;
}

static uint32_t ImGui_ImplVulkanHeap_MemoryType(VkMemoryPropertyFlags properties, uint32_t type_bits)
{
...
static void CreateOrResizeBuffer(VkBuffer& buffer, VkDeviceMemory& buffer_memory, VkDeviceSize& buffer_size, VkDeviceSize new_size, VkBufferUsageFlagBits usage)
{
    ...
    alloc_info.memoryTypeIndex = ImGui_ImplVulkanHeap_MemoryType(VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, req.memoryTypeBits);
...
static void ImGui_ImplVulkanHeap_SetupRenderState(ImDrawData* draw_data, VkPipeline pipeline, VkCommandBuffer command_buffer, ImGui_ImplVulkanHeap_FrameRenderBuffers* rb, int fb_width, int fb_height)
{
    ...
    if (bd->HeapMode)
    {
        // VK_EXT_descriptor_heap: the shader's legacy push_constant block reads
        // push data at offset 0; the sampler index word follows it.
        VkPushDataInfoEXT push_info = {};
        push_info.sType = VK_STRUCTURE_TYPE_PUSH_DATA_INFO_EXT;
        push_info.offset = 0;
        push_info.data.address = constants;
        push_info.data.size = sizeof(float) * 4;
        bd->HeapContext->CmdPushData(command_buffer, &push_info);

        bd->CurrentSamplerHeapIndex = 0;
        push_info.offset = v->HeapInfo.SamplerPushOffset;
        push_info.data.address = &bd->CurrentSamplerHeapIndex;
        push_info.data.size = sizeof(uint32_t);
        bd->HeapContext->CmdPushData(command_buffer, &push_info);
        return;
    }
...
// Draw callbacks
static void ImGui_ImplVulkanHeap_DrawCallback_ResetRenderState(const ImDrawList*, const ImDrawCmd*)     {} // Intentionally empty. Used as an identifier for rendering loop to call its code. Simpler to implement this way.
static void ImGui_ImplVulkanHeap_DrawCallback_SetSamplerLinear(const ImDrawList*, const ImDrawCmd*)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    if (bd->HeapMode)
    {
        ...
        VkPushDataInfoEXT push_info = {};
        push_info.sType = VK_STRUCTURE_TYPE_PUSH_DATA_INFO_EXT;
        push_info.offset = bd->VulkanInitInfo.HeapInfo.SamplerPushOffset;
        push_info.data.address = &bd->CurrentSamplerHeapIndex;
        push_info.data.size = sizeof(uint32_t);
        bd->HeapContext->CmdPushData(bd->RenderState->CommandBuffer, &push_info);
        return;
    }
    vkCmdBindDescriptorSets(bd->RenderState->CommandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, bd->PipelineLayout, 1, 1, &bd->SamplerLinearDS, 0, nullptr);
}
static void ImGui_ImplVulkanHeap_DrawCallback_SetSamplerNearest(const ImDrawList*, const ImDrawCmd*)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    if (bd->HeapMode)
    {
        bd->CurrentSamplerHeapIndex = 1;
        VkPushDataInfoEXT push_info = {};
        push_info.sType = VK_STRUCTURE_TYPE_PUSH_DATA_INFO_EXT;
        push_info.offset = bd->VulkanInitInfo.HeapInfo.SamplerPushOffset;
        push_info.data.address = &bd->CurrentSamplerHeapIndex;
...
// Render function
void ImGui_ImplVulkanHeap_RenderDrawData(ImDrawData* draw_data, VkCommandBuffer command_buffer, VkPipeline pipeline)
{
    ...
    if (draw_data->Textures != nullptr)
        for (ImTextureData* tex : *draw_data->Textures)
            if (tex->Status != ImTextureStatus_OK)
                ImGui_ImplVulkanHeap_UpdateTexture(tex);
    ...
    ImGui_ImplVulkanHeap_WindowRenderBuffers* wrb = &bd->MainWindowRenderBuffers;
    ...
    ImGui_ImplVulkanHeap_FrameRenderBuffers* rb = &wrb->FrameRenderBuffers[wrb->Index];
    ...
    ImGui_ImplVulkanHeap_RenderState render_state;
    ...
    ImGui_ImplVulkanHeap_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width, fb_height);
    ...
    for (const ImDrawList* draw_list : draw_data->CmdLists)
    {
        for (int cmd_i = 0; cmd_i < draw_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            if (pcmd->UserCallback != nullptr)
            {
                ...
                if (pcmd->UserCallback == ImGui_ImplVulkanHeap_DrawCallback_ResetRenderState)
                {
                    ImGui_ImplVulkanHeap_SetupRenderState(draw_data, pipeline, command_buffer, rb, fb_width, fb_height);
            ...
            else
            {
                ...
                if (bd->HeapMode)
                {
                    // VK_EXT_descriptor_heap: push the texture slot index word.
                    const uint32_t slot = (uint32_t)(uintptr_t)image_view;
                    VkPushDataInfoEXT push_info = {};
                    push_info.sType = VK_STRUCTURE_TYPE_PUSH_DATA_INFO_EXT;
                    push_info.offset = v->HeapInfo.PushIndexOffset;
                    push_info.data.address = &slot;
                    push_info.data.size = sizeof(uint32_t);
                    bd->HeapContext->CmdPushData(command_buffer, &push_info);
                }
                else if (image_view != last_image_view)
                {
...
static void ImGui_ImplVulkanHeap_DestroyTexture(ImTextureData* tex)
{
    if (ImGui_ImplVulkanHeap_Texture* backend_tex = (ImGui_ImplVulkanHeap_Texture*)tex->BackendUserData)
    {
        IM_ASSERT(backend_tex->DescriptorSet == (VkDescriptorSet)tex->TexID);
        ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
        ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
        ImGui_ImplVulkanHeap_RemoveTexture(backend_tex->DescriptorSet);
...
void ImGui_ImplVulkanHeap_UpdateTexture(ImTextureData* tex)
{
    if (tex->Status == ImTextureStatus_OK)
        return;
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ...
    if (tex->Status == ImTextureStatus_WantCreate)
    {
        ...
        ImGui_ImplVulkanHeap_Texture* backend_tex = IM_NEW(ImGui_ImplVulkanHeap_Texture)();
        ...
        // Create the Image:
        {
            ...
            alloc_info.memoryTypeIndex = ImGui_ImplVulkanHeap_MemoryType(VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, req.memoryTypeBits);
        ...
        // Create the Image View:
        {
            ...
            check_vk_result(err);
            backend_tex->ViewInfo = info; // VK_EXT_descriptor_heap: heap writes consume create infos
        }

        // Create the Descriptor Set / heap slot
        backend_tex->DescriptorSet = ImGui_ImplVulkanHeap_AddTextureEx(
            backend_tex->ImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL, &backend_tex->ViewInfo
        );
    ...
    {
        ImGui_ImplVulkanHeap_Texture* backend_tex = (ImGui_ImplVulkanHeap_Texture*)tex->BackendUserData;
    ...
    if (tex->Status == ImTextureStatus_WantDestroy && tex->UnusedFrames >= (int)bd->VulkanInitInfo.ImageCount)
        ImGui_ImplVulkanHeap_DestroyTexture(tex);
}

static void ImGui_ImplVulkanHeap_CreateShaderModules(VkDevice device, const VkAllocationCallbacks* allocator)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
...
static VkPipeline ImGui_ImplVulkanHeap_CreatePipeline(VkDevice device, const VkAllocationCallbacks* allocator, VkPipelineCache pipelineCache, const ImGui_ImplVulkanHeap_PipelineInfo* info)
{
    ...
    // --- VK_EXT_descriptor_heap: map set 0 (sampled image) and set 1 (sampler)
    // onto the heaps; both indices come from push data. ---
    VkDescriptorSetAndBindingMappingEXT heap_mappings[2] = {};
    VkShaderDescriptorSetAndBindingMappingInfoEXT heap_mapping_info = {};
    VkPipelineCreateFlags2CreateInfoKHR heap_flags2 = {};
    if (bd->HeapMode)
    {
        heap_mappings[0].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_AND_BINDING_MAPPING_EXT;
        heap_mappings[0].descriptorSet = 0;
        heap_mappings[0].firstBinding = 0;
        heap_mappings[0].bindingCount = 1;
        heap_mappings[0].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLED_IMAGE_BIT_EXT;
        heap_mappings[0].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT;
        heap_mappings[0].sourceData.pushIndex.heapOffset = v->HeapInfo.ResourceSlotBase * v->HeapInfo.ResourceStride;
        heap_mappings[0].sourceData.pushIndex.pushOffset = v->HeapInfo.PushIndexOffset;
        heap_mappings[0].sourceData.pushIndex.heapIndexStride = v->HeapInfo.ResourceStride;

        heap_mappings[1].sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_AND_BINDING_MAPPING_EXT;
        heap_mappings[1].descriptorSet = 1;
        heap_mappings[1].firstBinding = 0;
        heap_mappings[1].bindingCount = 1;
        heap_mappings[1].resourceMask = VK_SPIRV_RESOURCE_TYPE_SAMPLER_BIT_EXT;
        heap_mappings[1].source = VK_DESCRIPTOR_MAPPING_SOURCE_HEAP_WITH_PUSH_INDEX_EXT;
        heap_mappings[1].sourceData.pushIndex.heapOffset = v->HeapInfo.SamplerSlotLinear * v->HeapInfo.SamplerStride;
        heap_mappings[1].sourceData.pushIndex.pushOffset = v->HeapInfo.SamplerPushOffset;
        heap_mappings[1].sourceData.pushIndex.heapIndexStride = v->HeapInfo.SamplerStride;

        heap_mapping_info.sType = VK_STRUCTURE_TYPE_SHADER_DESCRIPTOR_SET_AND_BINDING_MAPPING_INFO_EXT;
        heap_mapping_info.mappingCount = 2;
        heap_mapping_info.pMappings = heap_mappings;

        stage[0].pNext = &heap_mapping_info;
        stage[1].pNext = &heap_mapping_info;

        heap_flags2.sType = VK_STRUCTURE_TYPE_PIPELINE_CREATE_FLAGS_2_CREATE_INFO_KHR;
        heap_flags2.flags = VK_PIPELINE_CREATE_2_DESCRIPTOR_HEAP_BIT_EXT;
    }
    ...
    create_info.layout = bd->HeapMode ? VK_NULL_HANDLE : bd->PipelineLayout; // VUID-VkGraphicsPipelineCreateInfo-flags-11311
    ...
    if (bd->HeapMode)
    {
        // VK_EXT_descriptor_heap: chain the heap pipeline flag in front of the
        // dynamic-rendering info.
        heap_flags2.pNext = create_info.pNext;
        create_info.pNext = &heap_flags2;
    }
#endif
    VkPipeline pipeline;
...
static void ImGui_ImplVulkanHeap_CreateDescriptorSetLayout(VkDescriptorSetLayout* p_layout, VkDescriptorType descriptor_type)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
    if (bd->HeapMode)
    {
        *p_layout = VK_NULL_HANDLE; // Descriptor heaps replace set layouts.
        return;
    }
...
static VkDescriptorSet ImGui_ImplVulkanHeap_CreateSamplerDS(VkSampler sampler)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;

    // VK_EXT_descriptor_heap: write the sampler descriptor into its static
    // sampler-heap slot (linear or nearest, distinguished by the sampler CI).
    if (bd->HeapMode)
    {
        const VkSamplerCreateInfo* info =
            (sampler == bd->SamplerNearest) ? &bd->SamplerNearestInfo : &bd->SamplerLinearInfo;
        const uint32_t slot =
            (sampler == bd->SamplerNearest) ? v->HeapInfo.SamplerSlotNearest : v->HeapInfo.SamplerSlotLinear;
        bd->HeapManager->WriteSampler(ZHLN::Vk::SamplerHandle {slot}, *info);
...
bool ImGui_ImplVulkanHeap_CreateDeviceObjects()
{
    ...
    if (!bd->DescriptorSetLayoutTexture)
        ImGui_ImplVulkanHeap_CreateDescriptorSetLayout(&bd->DescriptorSetLayoutTexture, VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE);
    if (!bd->DescriptorSetLayoutSampler)
        ImGui_ImplVulkanHeap_CreateDescriptorSetLayout(&bd->DescriptorSetLayoutSampler, VK_DESCRIPTOR_TYPE_SAMPLER);

    if (!bd->HeapMode && v->DescriptorPoolSize != 0)
    {
    ...
    if (!bd->SamplerLinear || !bd->SamplerNearest)
    {
        ...
        bd->SamplerLinearInfo = info;
        err = vkCreateSampler(v->Device, &info, v->Allocator, &bd->SamplerLinear);
        check_vk_result(err);
        bd->SamplerLinearDS = ImGui_ImplVulkanHeap_CreateSamplerDS(bd->SamplerLinear);

        info.magFilter = VK_FILTER_NEAREST;
        info.minFilter = VK_FILTER_NEAREST;
        info.mipmapMode = VK_SAMPLER_MIPMAP_MODE_NEAREST;
        bd->SamplerNearestInfo = info;
        err = vkCreateSampler(v->Device, &info, v->Allocator, &bd->SamplerNearest);
        check_vk_result(err);
        bd->SamplerNearestDS = ImGui_ImplVulkanHeap_CreateSamplerDS(bd->SamplerNearest);
    }

    if (!bd->HeapMode && !bd->PipelineLayout)
    {
    ...
    if (create_main_pipeline)
        ImGui_ImplVulkanHeap_CreateMainPipeline(&v->PipelineInfoMain);
...
void ImGui_ImplVulkanHeap_CreateMainPipeline(const ImGui_ImplVulkanHeap_PipelineInfo* pipeline_info_in)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
    if (bd->Pipeline)
    ...
    ImGui_ImplVulkanHeap_PipelineInfo* pipeline_info = &v->PipelineInfoMain;
...
#endif
    bd->Pipeline = ImGui_ImplVulkanHeap_CreatePipeline(v->Device, v->Allocator, v->PipelineCache, pipeline_info);
}
...
void    ImGui_ImplVulkanHeap_DestroyDeviceObjects()
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
    ImGui_ImplVulkanHeap_DestroyWindowRenderBuffers(v->Device, &bd->MainWindowRenderBuffers, v->Allocator);
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
static void ImGui_ImplVulkanHeap_LoadDynamicRenderingFunctions(uint32_t api_version, PFN_vkVoidFunction(*loader_func)(const char* function_name, void* user_data), void* user_data)
{
...
static uint32_t ImGui_ImplVulkanHeap_GetDefaultApiVersion()
{
...
bool    ImGui_ImplVulkanHeap_LoadFunctions(uint32_t api_version, PFN_vkVoidFunction(*loader_func)(const char* function_name, void* user_data), void* user_data)
{
    // Load function pointers
    // You can use the default Vulkan loader using:
    //      ImGui_ImplVulkanHeap_LoadFunctions(VK_API_VERSION_1_3, [](const char* function_name, void*) { return vkGetInstanceProcAddr(your_vk_isntance, function_name); });
    // But this would be roughly equivalent to not setting VK_NO_PROTOTYPES.
    if (api_version == 0)
        api_version = ImGui_ImplVulkanHeap_GetDefaultApiVersion();
...
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
    ImGui_ImplVulkanHeap_LoadDynamicRenderingFunctions(api_version, loader_func, user_data);
#endif
#else
...
bool    ImGui_ImplVulkanHeap_Init(ImGui_ImplVulkanHeap_InitInfo* info)
{
    IM_ASSERT(g_FunctionsLoaded && "Need to call ImGui_ImplVulkanHeap_LoadFunctions() if IMGUI_IMPL_VULKAN_NO_PROTOTYPES or VK_NO_PROTOTYPES are set!");

    if (info->ApiVersion == 0)
        info->ApiVersion = ImGui_ImplVulkanHeap_GetDefaultApiVersion();

    if (info->UseDynamicRendering)
    {
#ifdef IMGUI_IMPL_VULKAN_HAS_DYNAMIC_RENDERING
#ifndef IMGUI_IMPL_VULKAN_USE_LOADER
        ImGui_ImplVulkanHeap_LoadDynamicRenderingFunctions(info->ApiVersion, [](const char* function_name, void* user_data) { return vkGetDeviceProcAddr((VkDevice)user_data, function_name); }, (void*)info->Device);
#endif
    ...
    ImGui_ImplVulkanHeap_Data* bd = IM_NEW(ImGui_ImplVulkanHeap_Data)();
    io.BackendRendererUserData = (void*)bd;
    io.BackendRendererName = "imgui_impl_vulkan_heap";
    io.BackendFlags |= ImGuiBackendFlags_RendererHasVtxOffset;  // We can honor the ImDrawCmd::VtxOffset field, allowing for large meshes.
    io.BackendFlags |= ImGuiBackendFlags_RendererHasTextures;   // We can honor ImGuiPlatformIO::Textures[] requests during render.

    ImGuiPlatformIO& platform_io = ImGui::GetPlatformIO();
    platform_io.DrawCallback_ResetRenderState = ImGui_ImplVulkanHeap_DrawCallback_ResetRenderState;
    platform_io.DrawCallback_SetSamplerLinear = ImGui_ImplVulkanHeap_DrawCallback_SetSamplerLinear;
    platform_io.DrawCallback_SetSamplerNearest = ImGui_ImplVulkanHeap_DrawCallback_SetSamplerNearest;
    ...
    if (info->DescriptorPool != VK_NULL_HANDLE) // Either DescriptorPool or DescriptorPoolSize must be set, not both!
        ...
        if (info->HeapInfo.ResourceSlotCount == 0)
            IM_ASSERT(info->DescriptorPoolSize > 0);
    if (info->UseDynamicRendering)
        IM_ASSERT(info->PipelineInfoMain.RenderPass == VK_NULL_HANDLE);

    bd->VulkanInitInfo = *info;
    bd->HeapMode = (info->HeapInfo.ResourceSlotCount > 0);
    if (bd->HeapMode)
    {
        bd->HeapContext = (const ZHLN::Vk::Context*) info->HeapInfo.HeapContext;
        bd->HeapManager = (ZHLN::Vk::HeapManager*) info->HeapInfo.HeapManager;
        IM_ASSERT(bd->HeapContext != nullptr && bd->HeapManager != nullptr);
    }
    ...
    if (!ImGui_ImplVulkanHeap_CreateDeviceObjects())
        IM_ASSERT(0 && "ImGui_ImplVulkanHeap_CreateDeviceObjects() failed!"); // <- Can't be hit yet.

    return true;
}

void ImGui_ImplVulkanHeap_Shutdown()
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ...
    ImGui_ImplVulkanHeap_DestroyDeviceObjects();
...
void ImGui_ImplVulkanHeap_NewFrame()
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    IM_ASSERT(bd != nullptr && "Context or backend not initialized! Did you call ImGui_ImplVulkanHeap_Init()?");
    IM_UNUSED(bd);
}

void ImGui_ImplVulkanHeap_SetMinImageCount(uint32_t min_image_count)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
...
VkDescriptorSet ImGui_ImplVulkanHeap_AddTextureEx(VkImageView image_view, VkImageLayout image_layout, const VkImageViewCreateInfo* image_view_info)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;

    // --- VK_EXT_descriptor_heap: allocate a slot from the ImGui region and
    // write the image descriptor into the resource heap. The slot index is the
    // ImTextureID. ---
    if (bd->HeapMode)
    {
        uint32_t slot = 0;
        if (!bd->FreeHeapTextureSlots.empty())
        {
            slot = bd->FreeHeapTextureSlots.back();
            bd->FreeHeapTextureSlots.pop_back();
        }
        else if (bd->NextHeapTextureSlot < v->HeapInfo.ResourceSlotCount)
        {
            slot = bd->NextHeapTextureSlot++;
        }

        if (image_view_info != nullptr)
        {
            bd->HeapManager->WriteImage(ZHLN::Vk::TextureHandle {v->HeapInfo.ResourceSlotBase + slot}, *image_view_info, image_layout);
        }
        return (VkDescriptorSet)(uintptr_t)slot;
    }
...
VkDescriptorSet ImGui_ImplVulkanHeap_AddTexture(VkImageView image_view, VkImageLayout image_layout)
{
    return ImGui_ImplVulkanHeap_AddTextureEx(image_view, image_layout, nullptr);
}

#ifndef IMGUI_DISABLE_OBSOLETE_FUNCTIONS
VkDescriptorSet ImGui_ImplVulkanHeap_AddTexture(VkSampler sampler, VkImageView image_view, VkImageLayout image_layout)
{
    IM_UNUSED(sampler);
    return ImGui_ImplVulkanHeap_AddTexture(image_view, image_layout);
}
#endif

void ImGui_ImplVulkanHeap_RemoveTexture(VkDescriptorSet descriptor_set)
{
    ImGui_ImplVulkanHeap_Data* bd = ImGui_ImplVulkanHeap_GetBackendData();
    ImGui_ImplVulkanHeap_InitInfo* v = &bd->VulkanInitInfo;
    if (bd->HeapMode)
    {
        const uint32_t slot = (uint32_t)(uintptr_t)descriptor_set;
        if (slot != 0 && slot < v->HeapInfo.ResourceSlotCount)
            bd->FreeHeapTextureSlots.push_back(slot);
        return;
    }
    VkDescriptorPool pool = bd->DescriptorPool ? bd->DescriptorPool : v->DescriptorPool;
...
void ImGui_ImplVulkanHeap_DestroyWindowRenderBuffers(VkDevice device, ImGui_ImplVulkanHeap_WindowRenderBuffers* buffers, const VkAllocationCallbacks* allocator)
{
    for (uint32_t n = 0; n < buffers->Count; n++)
        ImGui_ImplVulkanHeap_DestroyFrameRenderBuffers(device, &buffers->FrameRenderBuffers[n], allocator);
...
// (The ImGui_ImplVulkanH_XXX functions do not interact with any of the state used by the regular ImGui_ImplVulkanHeap_XXX functions)
//-------------------------------------------------------------------------
...
void ImGui_ImplVulkanH_CreateWindowSwapChain(VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, const VkAllocationCallbacks* allocator, int w, int h, uint32_t min_image_count, VkImageUsageFlags image_usage)
{
    ...
    if (wd->UseDynamicRendering == false)
    {
        ...
        //ImGui_ImplVulkanHeap_CreatePipeline(device, allocator, VK_NULL_HANDLE, wd->RenderPass, VK_SAMPLE_COUNT_1_BIT, &wd->Pipeline, v->Subpass);
    }
...
void ImGui_ImplVulkanH_CreateOrResizeWindow(VkInstance instance, VkPhysicalDevice physical_device, VkDevice device, ImGui_ImplVulkanH_Window* wd, uint32_t queue_family, const VkAllocationCallbacks* allocator, int width, int height, uint32_t min_image_count, VkImageUsageFlags image_usage)
{
    ...
    // before the ImGui_ImplVulkanHeap_Init function, so we don't have access to the queue yet. Here we have the queue_family that we can use to grab
