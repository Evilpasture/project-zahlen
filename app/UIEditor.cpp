...
#if defined(ZHLN_HAS_UI_TOML)
void SaveTree(const GUI::UINode& tree) {
    const std::string text = ZHLN::ReflectTOML::SerializeTOML(tree);
    std::ofstream     out {std::string {kDocumentPath}, std::ios::binary | std::ios::trunc};
    if (!out) {
        ZHLN::Log("[UIEditor] could not open '{}' for writing", kDocumentPath);
        return;
    }
    out.write(text.data(), static_cast<std::streamsize>(text.size()));
    out.close();
    if (out.fail()) {
        ZHLN::Log("[UIEditor] writing '{}' failed", kDocumentPath);
        return;
    }
    ZHLN::Log("[UIEditor] wrote '{}'", kDocumentPath);
}

void LoadTree(Session& session) {
    std::ifstream in {std::string {kDocumentPath}, std::ios::binary};
    if (!in) {
        ZHLN::Log("[UIEditor] '{}' is not readable", kDocumentPath);
        return;
    }
    const std::string text {std::istreambuf_iterator<char>(in), std::istreambuf_iterator<char>()};
    auto              parsed = ZHLN::ReflectTOML::TryParse<GUI::UINode>(text);
    if (!parsed) {
        ZHLN::Log("[UIEditor] '{}' did not parse as a UINode", kDocumentPath);
        return;
    }
    session.tree       = std::move(*parsed);
    session.selectedId = RootPath(session.tree);
    ZHLN::Log("[UIEditor] loaded '{}'", kDocumentPath);
}
#endif
