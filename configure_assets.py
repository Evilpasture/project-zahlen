...
"""
    ...
    for blend_path in blend_files:
        level = os.path.splitext(os.path.basename(blend_path))[0]
        ...
        ninja_content += f"\nbuild {escape_ninja(meta_path)}: blender_extract {escape_ninja(blend_path)}\n"

        # Safe bootstrap: If manifest JSON exists, parse and generate asset pipelines.
        # If it doesn't, we skip this pass. After Ninja runs Blender, the self-regeneration
        # loop will immediately reload the generated targets.
    ...
    # If any .blend file, configure_assets.py, or metadata.json changes, assets.ninja regenerates
    ...
    ninja_content += f"\nbuild {escaped_output}: regenerate_ninja {escaped_configure} | {blend_deps} {meta_deps}\n"
