...
class Queue {
  ...
  public:
    Queue(): _capacity(InitialCapacity) {
        AllocateStorage(_capacity);
    }
    ...
    Queue(const Queue& other): _tail(other._size), _capacity(other._capacity), _size(other._size) {
        ...
        _data = static_cast<T*>(::operator new[](_capacity * sizeof(T), std::align_val_t {alignof(T)}, std::nothrow));
    ...
    Queue& operator=(const Queue& other) {
        if (this != &other) {
            ...
            T* newData = static_cast<T*>(::operator new[](other._capacity * sizeof(T), std::align_val_t {alignof(T)}, std::nothrow));
  ...
  private:
    ...
    void AllocateStorage(size_t cap) {
        ...
        _data = static_cast<T*>(::operator new[](cap * sizeof(T), std::align_val_t {alignof(T)}, std::nothrow));
        if (_data == nullptr) [[unlikely]] {
            TrapAllocationFailure();
        }
    }
    ...
    void Grow() {
        ...
        T* newData = static_cast<T*>(::operator new[](newCapacity * sizeof(T), std::align_val_t {alignof(T)}, std::nothrow));
        if (newData == nullptr) [[unlikely]] {
            TrapAllocationFailure();
        }
