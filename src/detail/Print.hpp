...
class SignalSafePool {
  public:
	...
	static char* GetBufferMutable(size_t idx) noexcept { return s_pool[idx]; }
...
class FormatResult {
  public:
	...
	constexpr FormatResult(size_t poolIdx, size_t len) noexcept
		: _poolIdx(poolIdx), _len(len), _valid(true) {}
	...
	FormatResult(FormatResult&& other) noexcept
		: _poolIdx(other._poolIdx), _len(other._len), _valid(other._valid) {
		other._valid = false;
	}
	...
	[[nodiscard]] std::string_view string_view() const noexcept {
		return _valid ? std::string_view(SignalSafePool::GetBuffer(_poolIdx), _len)
					  : std::string_view();
	}

	operator std::string_view() const noexcept { return string_view(); }

	[[nodiscard]] size_t size() const noexcept { return _len; }
 ...
 */
inline size_t FormatUInt(char* buf, size_t max_len, uint64_t val, int base = 10,
...
template <typename T> inline size_t AppendValue(char* buf, size_t max_len, const T& val) noexcept {
	if constexpr (std::is_same_v<std::decay_t<T>, int> || std::is_same_v<std::decay_t<T>, long> ||
	...
	if constexpr (argCount == 0) {
		...
		auto EraseOne = [&]<typename T>(const T& val) {
			erasedArgs[trackedCount++] = {
				...
				.func = [](const void* ptr, char* b, size_t len,
						   Detail::FormatOptions opts) -> size_t {
					using DecayedT = std::decay_t<T>;
					if constexpr (std::is_integral_v<DecayedT> && !std::is_same_v<DecayedT, bool> &&
								  !std::is_same_v<DecayedT, char>) {
						if (opts.hex) {
							return Detail::FormatUInt(
								b, len, static_cast<uint64_t>(*static_cast<const T*>(ptr)), 16,
								opts.uppercase);
						}
					}
		...
		auto BoundAppenders = [&](size_t idx, char* b, size_t len,
								  Detail::FormatOptions opts) -> size_t {
			if (idx >= argCount) {
				return 0;
			}
...
template <typename... Args> inline void Print(std::string_view fmt, Args&&... args) noexcept {
...
template <typename... Args> inline void Println(std::string_view fmt, Args&&... args) noexcept {
