...
void AudioContext::UpdateListener(const JPH::Vec3& position, const JPH::Vec3& direction,
								  const JPH::Vec3& up) {
	if (!_impl->initialized) {
		return;
	}
...
void AudioContext::PlayOneShot3D(const std::string& filepath, const JPH::Vec3& position,
								 float volume) {
	if (!_impl->initialized) {
		return;
	}
...
void ZHLN_PlayOneShot3D(ZHLN_Engine* engine_handle, const char* filepath, float x, float y, float z,
						float volume) {
	auto* engine = reinterpret_cast<ZHLN::Engine*>(engine_handle);
	engine->GetAudioContext().PlayOneShot3D(filepath, JPH::Vec3(x, y, z), volume);
}

void ZHLN_PlayProceduralBeep(ZHLN_Engine* engine_handle, float frequency, float duration,
							 float volume) {
	auto* engine = reinterpret_cast<ZHLN::Engine*>(engine_handle);
