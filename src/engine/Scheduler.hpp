...
struct TaskSystemScheduler {
	template <typename... Tasks> void Dispatch(Tasks&&... tasks) const {
		...
		((fiberTasks[idx] = TaskSystem::Task{.func =
												 [](void* arg) {
													 using DecayedTask =
														 std::decay_t<decltype(tasks)>;
													 auto* taskPtr = static_cast<DecayedTask*>(arg);
													 (*taskPtr)();
												 },
											 .arg = const_cast<void*>(
												 static_cast<const void*>(std::addressof(tasks)))},
		  ++idx),
		 ...);
