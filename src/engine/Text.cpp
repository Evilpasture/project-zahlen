...
auto CreateTextMesh(RenderContext& ctx, const FontAtlas& font, const std::string& text, float x, float y, float scale, const JPH::Vec4& color) -> Mesh {
    if (text.empty()) {
        return {};
    }
    ...
    for (char c: text) {
        ...
        VertexAttributes vTL_attr = {.normal = dummyNormal, .tangent = dummyTangent, .uv = Math::PackUV(u0, v0), .color = packedColor};

        VertexPosition   vTR_pos  = {{x1, y0, 0.0f}};
        VertexAttributes vTR_attr = {.normal = dummyNormal, .tangent = dummyTangent, .uv = Math::PackUV(u1, v0), .color = packedColor};

        VertexPosition   vBL_pos  = {{x0, y1, 0.0f}};
        VertexAttributes vBL_attr = {.normal = dummyNormal, .tangent = dummyTangent, .uv = Math::PackUV(u0, v1), .color = packedColor};

        VertexPosition   vBR_pos  = {{x1, y1, 0.0f}};
        VertexAttributes vBR_attr = {.normal = dummyNormal, .tangent = dummyTangent, .uv = Math::PackUV(u1, v1), .color = packedColor};
    ...
    BufferHandle posVbo  = ctx.CreateVertexBuffer(positions.data(), positions.size() * sizeof(VertexPosition), sizeof(VertexPosition));
    BufferHandle attrVbo = ctx.CreateVertexBuffer(attributes.data(), attributes.size() * sizeof(VertexAttributes), sizeof(VertexAttributes));
...
auto CreatePanelMesh(RenderContext& ctx, const Components::UIRectComponent& rect, const Components::UIPanelComponent& panel) -> Mesh {
    ...
    if (panel.edgeWidth > 0.0f && width > 0.0f && height > 0.0f) {
        ...
        for (int r = 0; r < 3; ++r) {
            for (int col = 0; col < 3; ++col) {
                ...
                attributes.push_back({.normal = n, .tangent = t, .uv = Math::PackUV(qu0, qv0), .color = c});

                positions.push_back({{qx0, qy1, 0.0f}});
                attributes.push_back({.normal = n, .tangent = t, .uv = Math::PackUV(qu0, qv1), .color = c});

                positions.push_back({{qx1, qy0, 0.0f}});
                attributes.push_back({.normal = n, .tangent = t, .uv = Math::PackUV(qu1, qv0), .color = c});
                ...
                attributes.push_back({.normal = n, .tangent = t, .uv = Math::PackUV(qu1, qv1), .color = c});
            }
        }
    } else {
    ...
    return Mesh {.posBuffer = posVbo, .attrBuffer = attrVbo, .vertexCount = static_cast<uint32_t>(positions.size()), .indexCount = 0};
}
