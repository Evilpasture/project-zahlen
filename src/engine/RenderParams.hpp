...
struct AmbientPassParams {
	...
	[[nodiscard]] constexpr auto AsTuple() const noexcept {
		return std::tuple{sceneColor,  defaultSampler, depth,
						  normRough,   pointSampler,   prefilteredView,
						  brdfLutView, clampSampler,   frameUniformBuffer};
	}
};
...
struct LightingPassParams {
	...
	[[nodiscard]] constexpr auto AsTuple() const noexcept {
		return std::tuple{
			sceneColor,			  defaultSampler,	  depth,		 normRough,
			lightStorageBuffer,	  frameUniformBuffer, shadowMapView, shadowSampler,
			ltcMatView,			  ltcAmpView,		  clampSampler,	 clusterGridBuffer,
			lightIndexListBuffer, ambientTarget,	  pointSampler,	 tlas,
			shadowAtlasCubeView,  shadowAtlas2DView};
	}
};
...
struct ReflectionPassParams {
	...
	[[nodiscard]] constexpr auto AsTuple() const noexcept {
		return std::tuple{sceneColor,	defaultSampler,	 depth,			normRough,
						  pointSampler, prefilteredView, tlas,			frameUniformBuffer,
						  brdfLutView,	clampSampler,	 lightingTarget};
	}
};
