...
class ZHLN_API RenderSystem {
  public:
    ...
    static std::expected<void, Error> Update(Engine& engine, float dt);

  private:
    // Phase 1: Resolves viewports, cascaded shadows, and draws standard mesh components
    static std::expected<void, Error> RenderMain(Engine& engine, int& outPhysicsDrawMode, JPH::Mat44& outShadowProjView, float dt);
