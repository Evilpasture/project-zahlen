...
static void PerformDiagnosticDump(int sig, void* addr, Engine* engine) {
    ...
    if (addr != nullptr) {
        ...
        for (size_t i = 0; i < totalSize; i += bytesPerLine) {
            ...
            lineBuf[offset] = '\0';
            WriteToChannel(static_cast<uint8_t>(LogChannel::StdErr), std::string_view(lineBuf, offset));
            WriteToChannel(static_cast<uint8_t>(LogChannel::StdErr), " | ");
            ...
            WriteToChannel(static_cast<uint8_t>(LogChannel::StdErr), std::string_view(ascii_buf, bytesPerLine));
            WriteToChannel(static_cast<uint8_t>(LogChannel::StdErr), " |\n");
        }
        WriteToChannel(static_cast<uint8_t>(LogChannel::StdErr), "\n");
    }
    ...
    if (engine != nullptr) {
        ...
        for (int i = 0; i < 6; ++i) {
            ...
            auto plane_str = ZHLN::Format(
                "  Plane {}: [{}x {}y {}z] offset: {}\n", names[i], f.mX[block].mF32[lane], f.mY[block].mF32[lane], f.mZ[block].mF32[lane],
                f.mW[block].mF32[lane]
            );
