...
class IBLProcessor {
  public:
	static IBLPayload Bake(RenderContext::Impl& impl, StagingContext& staging) {
		IBLPayload payload;
		...
		payload.brdfLutImage =
			Image::Create(impl.allocator.Get(), lutInfo, VMA_MEMORY_USAGE_GPU_ONLY);

		staging.UploadImage2D(payload.brdfLutImage.Handle(), 512, 512, 1, lutData.data(),
							  512 * 512 * 4);
		payload.brdfLutView =
			CreateView<VK_FORMAT_R8G8B8A8_UNORM>(impl.ctx.Device(), payload.brdfLutImage.Handle());
		...
		payload.prefilteredImage =
			Image::Create(impl.allocator.Get(), specInfo, VMA_MEMORY_USAGE_GPU_ONLY);
		...
		Buffer stagingBuf =
			Buffer::Create(impl.allocator.Get(), totalBytes, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
		...
		staging.UploadPrefilteredCubeMap(payload.prefilteredImage.Handle(), stagingBuf.Handle(),
										 kBaseSize, kMipLevels);
		staging.AddBuffer(std::move(stagingBuf));

		payload.prefilteredView = CreateViewCube<VK_FORMAT_R8G8B8A8_UNORM>(
			impl.ctx.Device(), payload.prefilteredImage.Handle(), kMipLevels);
		return payload;
	}
};
