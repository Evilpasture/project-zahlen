...
int CookAnimation(int argc, char** argv) {
    ...
    auto appendPayload = [&](const std::string& binFile, uint32_t offset, uint32_t length) -> uint32_t {
    ...
    for (const auto& channel: anim.channels) {
        ...
        track.valueOffset = appendPayload(sampler.binFile, sampler.outputOffset, sampler.outputLength);
...
int PackArchive(int argc, char** argv) {
        ...
        ZHLN::TaskSystem::ParallelFor(totalFiles, 1, [&](uint32_t start, uint32_t end, uint32_t) {
            for (uint32_t i = start; i < end; ++i) {
                ...
                if (current % 10 == 0 || current == totalFiles) {
                    ...
                    std::print("\r[zcook] Loading assets: {}/{} ({:.1f}%)", current, totalFiles, (static_cast<float>(current) / totalFiles) * 100.0f);
                    std::fflush(stdout);
                }
            }
        });
    ...
    auto updateProgress = [&](bool force) {
        ...
        if (force || std::chrono::duration_cast<std::chrono::milliseconds>(now - lastProgressTime).count() >= 100) {
            double writtenMB  = totalBytesWritten / 1048576.0;
            double totalMB    = totalBytesToWrite / 1048576.0;
            double percentage = totalBytesToWrite > 0 ? (totalBytesWritten * 100.0 / totalBytesToWrite) : 100.0;
            double speedMBs   = (std::chrono::duration_cast<std::chrono::milliseconds>(now - startWriteTime).count() > 0) ?
                                    (writtenMB / std::chrono::duration_cast<std::chrono::milliseconds>(now - startWriteTime).count() * 1000.0) :
                                    0.0;
            std::print("\r[zcook] Writing archive: {:.2f} / {:.2f} MB ({:.1f}%) | {:.1f} MB/s", writtenMB, totalMB, percentage, speedMBs);
            std::fflush(stdout);
    ...
    std::println("[zcook] Successfully packed {}/{} assets into '{}'", entries.size(), totalFiles, outPath);
    return 0;
}
