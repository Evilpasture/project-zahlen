...
Plan SolvePlan(const PlanRequest& request, const std::vector<Action>& actions) {
    ...
    pool[0] = GOAPNode {
        .state = request.current, .g_score = 0, .f_score = CalculateHeuristic(request.current, request.goal), .parent_idx = 0xFFFFFFFF, .action_idx = 0xFFFFFFFF
    };
    pool_count++;
    heap.Push(0);
    ...
    while (!heap.Empty()) {
        ...
        for (uint32_t i = 0; i < static_cast<uint32_t>(actions.size()); ++i) {
            ...
            for (uint32_t j = 0; j < pool_count; ++j) {
                if (pool[j].state.values == next_state.values && pool[j].state.mask == next_state.mask) {
                    state_exists = true;
                    if (tentative_g < pool[j].g_score) {
                        pool[j].g_score    = tentative_g;
                        pool[j].f_score    = tentative_g + CalculateHeuristic(next_state, request.goal);
