...
auto UnsafeReflectedLayoutBuilder::BuildUnsafe(VkDevice device) noexcept -> UnsafeReflectedLayout {
    ...
    for (uint32_t i = 0; i < _stageCount; ++i) {
        ...
        SpvReflectResult       reflectResult = spvReflectCreateShaderModule(stage.size, stage.code, &module);
        ...
        for (const auto* reflectedSet: sets) {
            ...
            for (uint32_t b = 0; b < reflectedSet->binding_count; ++b) {
                ...
                if (reflectedBinding->count >= 1024) {
                    mergedFlags[setIdx][bindingIdx] = VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT | VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT;
                }
            }
        }
    ...
    for (const auto& [setIdx, bindingsMap]: mergedSets) {
        ...
        const VkDescriptorSetLayoutCreateInfo createInfo = {
            ...
            .flags        = hasBindlessFlags ? (VkDescriptorSetLayoutCreateFlags) VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT : 0u,
