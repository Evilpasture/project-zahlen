...
void RayTracingContext::GetBlasSizes(const ZHLN_BlasGeometryDesc& desc, uint32_t primCount,
									 ZHLN_AccelerationStructureSizes& outSizes) const noexcept {
	ZHLN_GetBlasSizes(&_raw, &desc, primCount, &outSizes);
}

void RayTracingContext::GetTlasSizes(uint32_t instanceCount,
									 ZHLN_AccelerationStructureSizes& outSizes) const noexcept {
...
void RayTracingContext::CmdBuildBlas(VkCommandBuffer cmd, const ZHLN_BlasGeometryDesc& desc,
...
void RayTracingContext::CmdBuildTlas(VkCommandBuffer cmd, const ZHLN_TlasGeometryDesc& desc,
									 VkAccelerationStructureKHR dst, VkDeviceAddress scratch,
