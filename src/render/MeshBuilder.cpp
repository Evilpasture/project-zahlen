...
auto CreateTetrahedronMesh(RenderContext& ctx) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreateTetrahedronMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
...
auto CreatePlaneMesh(RenderContext& ctx, float extent, const JPH::Vec4& color) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreatePlaneMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
...
auto CreateBoxMesh(RenderContext& ctx, JPH::Vec3Arg halfExtents, const JPH::Vec4& color) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreateBoxMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
...
auto CreateSphereMesh(RenderContext& ctx, float radius, const JPH::Vec4& color) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreateSphereMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
...
auto CreateCylinderMesh(RenderContext& ctx, float radius, float height, const JPH::Vec4& color) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreateCylinderMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
...
auto CreateConeMesh(RenderContext& ctx, float radius, float height, const JPH::Vec4& color) -> Mesh {
    ...
    if (!res) [[unlikely]] {
        if (!res.error().Is(RenderFeatureError::FeatureNotSupported)) {
            ZHLN::Log("WARNING: CreateConeMesh: Failed to build mesh BLAS: {}", ZHLN::Error(res.error()).Message());
        }
    }
    return finalMesh;
}
