...
class DynamicPass {
  public:
    ...
    constexpr auto AddColor(
        const Vk::TypedImage<Layout>& img,
        VkAttachmentLoadOp            loadOp     = VK_ATTACHMENT_LOAD_OP_LOAD,
        VkAttachmentStoreOp           storeOp    = VK_ATTACHMENT_STORE_OP_STORE,
        const ZHLN::Color4&           clearColor = {.r = 0.0f, .g = 0.0f, .b = 0.0f, .a = 1.0f}
    ) && noexcept -> DynamicPass<ColorCount + 1, HasDepth> {
        static_assert(ColorCount < kMaxColorAttachments, "ZHLN Error: Legacy DynamicPass exceeded color attachment limits.");

        _colors[ColorCount] = {.view = img.view, .format = img.format, .loadOp = loadOp, .storeOp = storeOp, .clearColor = clearColor};
    ...
    constexpr auto AddDepth(
        ...
        static_assert(!HasDepth, "ZHLN Execution Error: Depth target already bound to legacy DynamicPass.");

        _depth = {.view = img.view, .format = img.format, .loadOp = loadOp, .storeOp = storeOp, .clearVal = clearVal};
    ...
    void Execute(VkCommandBuffer cmd, Func&& func) const {
        ...
        for (size_t i = 0; i < ColorCount; ++i) {
            ...
            VkClearValue cv {};
            cv.color = {{_colors[i].clearColor.r, _colors[i].clearColor.g, _colors[i].clearColor.b, _colors[i].clearColor.a}};
        ...
        VkViewport viewport {
            .x = 0.0f, .y = 0.0f, .width = static_cast<float>(_extent.width), .height = static_cast<float>(_extent.height), .minDepth = 0.0f, .maxDepth = 1.0f
        };
