...
struct RenderPassKey {
	...
	bool operator==(const RenderPassKey& other) const {
		return colorFormats == other.colorFormats && colorLoadOps == other.colorLoadOps &&
			   colorStoreOps == other.colorStoreOps && depthFormat == other.depthFormat &&
...
struct FramebufferKey {
	...
	bool operator==(const FramebufferKey& other) const {
		return renderPass == other.renderPass && attachments == other.attachments &&
...
class PassCache {
  public:
	static void Init(VkDevice device) { _device = device; }
	...
	static auto GetOrCreateRenderPass(const RenderPassKey& key) -> VkRenderPass {
		...
		for (size_t i = 0; i < key.colorFormats.size(); ++i) {
			...
			colorRefs.push_back({.attachment = static_cast<uint32_t>(attachments.size() - 1),
	...
	static auto GetOrCreateFramebuffer(const FramebufferKey& key) -> VkFramebuffer {
		...
		VkFramebufferCreateInfo createInfo{.sType = VK_STRUCTURE_TYPE_FRAMEBUFFER_CREATE_INFO,
										   ...
										   .attachmentCount =
											   static_cast<uint32_t>(key.attachments.size()),
  ...
  private:
	...
	inline static std::unordered_map<FramebufferKey, VkFramebuffer, FramebufferKeyHash>
		_framebuffers;
};
