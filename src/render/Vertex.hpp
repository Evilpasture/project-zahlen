...
[[nodiscard]] consteval auto MakeAttributeArray(Args... args) noexcept {
    constexpr size_t                                     count = sizeof...(Args);
    std::array<VkVertexInputAttributeDescription, count> attrs {};
    size_t                                               i = 0;
    ((attrs[i] = {.location = static_cast<uint32_t>(i), .binding = 0, .format = args.format, .offset = args.offset}, ++i), ...);
    return attrs;
}
...
template <typename T>
[[nodiscard]] consteval size_t GetFieldCount() noexcept {
    return std::meta::nonstatic_data_members_of(^^T, std::meta::access_context::unprivileged()).size();
}

// Isolate data extraction into a flat array structure with an explicit compile-time size
template <typename T, size_t N>
[[nodiscard]] consteval auto ExtractMetaElements() noexcept {
    auto         members = std::meta::nonstatic_data_members_of(^^T, std::meta::access_context::unprivileged());
    MetaRange<N> range {};
...
#define ZHLN_FIELD(Type, Mem)                                                                                            \
    ::ZHLN::Vk::MemberInfo {                                                                                             \
        .offset = static_cast<uint32_t>(offsetof(Type, Mem)), .format = ::ZHLN::Vk::FormatOf<decltype(Type::Mem)>::value \
    }
...
#define ZHLN_MAP_MEMBERS(Type, ...) \
    ZHLN_EXPAND(ZHLN_GET_MACRO(__VA_ARGS__, ZHLN_MAP_8, ZHLN_MAP_7, ZHLN_MAP_6, ZHLN_MAP_5, ZHLN_MAP_4, ZHLN_MAP_3, ZHLN_MAP_2, ZHLN_MAP_1)(Type, __VA_ARGS__))

#ifdef __cpp_impl_reflection
#define ZHLN_REFLECT_VERTEX(Type, ...) static_assert(::ZHLN::Vk::IsVertex<Type>, "Type '" #Type "' failed automatic C++26 vertex layout validation.");
#else
