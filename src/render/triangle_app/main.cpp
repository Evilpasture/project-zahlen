...
auto main() -> int {
    ...
    ZHLN_DeviceDesc dev_desc = {
        .extensions = dev_exts.data(), .extension_count = static_cast<uint32_t>(dev_exts.size()), .features = features.GetRoot(), .enable_validation = true
    };
    ...
    while (win.running) {
        ...
        Vk::DrawFrameDesc<3> frameDesc = {.ctx = ctx, .swapchain = pres.swapchain, .sync = sync, .pools = pools, .presentSemaphores = pres.presentSemaphores};
        ...
        Vk::DrawFrame(
            ...
            [&](VkCommandBuffer cmd, uint32_t image_index) {
                ...
                auto swap_att = Vk::Transition<VK_IMAGE_LAYOUT_COLOR_ATTACHMENT_OPTIMAL>(cmd, swapImage);

                Vk::DynamicPass(swap_att.extent)
                    .AddColor(swap_att, VK_ATTACHMENT_LOAD_OP_CLEAR, VK_ATTACHMENT_STORE_OP_STORE, {.r = 0.01f, .g = 0.01f, .b = 0.02f, .a = 1.0f})
                    .Execute(cmd, [&]() {
