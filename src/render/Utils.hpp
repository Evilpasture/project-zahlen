...
template <typename T> constexpr T Fract(T x) {
	return x - Floor(x);
}

// GLSL-style mix: Linear interpolation
template <typename T, typename U> constexpr T Mix(const T& a, const T& b, const U& t) {
	return a + t * (b - a);
}

// GLSL-style saturate: Clamps between 0 and 1
template <typename T> constexpr T Saturate(T x) {
...
template <typename BaseT, typename ExpT> constexpr auto Power(BaseT base, ExpT exp) noexcept {
...
[[nodiscard]] constexpr uint32_t PackColor(uint8_t r, uint8_t g, uint8_t b) noexcept {
	return 0xFF000000u | (static_cast<uint32_t>(b) << 16) | (static_cast<uint32_t>(g) << 8) |
...
template <typename T> [[nodiscard]] constexpr T Abs(T x) noexcept {
	return (x < 0) ? -x : x;
}
