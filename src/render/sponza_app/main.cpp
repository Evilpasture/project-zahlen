...
using SceneLayout = ZHLN::Vk::DescriptorLayout<
    ZHLN::Vk::BindlessSampledImageSlot<0, 4096>, // Global Textures Array
...
struct CreativeWorkPaths {
    ...
    [[nodiscard]] bool Validate() const {
        return FileExists(gltf) && FileExists(vert_spv) && FileExists(frag_spv) && FileExists(shadow_vert_spv) && FileExists(shadow_frag_spv);
    }
};
            ...
            for (cgltf_size a = 0; a < prim->attributes_count; ++a) {
                ...
                for (cgltf_size v = 0; v < vertexCount; ++v) {
                    if (attr->type == cgltf_attribute_type_position) {
                        cgltf_accessor_read_float(attr->data, v, scene.vertices[startVert + v].pos.data(), 3);
                    } else if (attr->type == cgltf_attribute_type_normal) {
                        cgltf_accessor_read_float(attr->data, v, scene.vertices[startVert + v].norm.data(), 3);
                    } else if (attr->type == cgltf_attribute_type_tangent) {
                        cgltf_accessor_read_float(attr->data, v, scene.vertices[startVert + v].tangent.data(), 4);
                    } else if (attr->type == cgltf_attribute_type_texcoord) {
                        if (attr->index == 0) { // UV0
                            cgltf_accessor_read_float(attr->data, v, scene.vertices[startVert + v].uv0.data(), 2);
                        } else if (attr->index == 1) { // UV1
                            cgltf_accessor_read_float(attr->data, v, scene.vertices[startVert + v].uv1.data(), 2);
                        }
                    }
                }
            }
...
static ZHLN::Vk::TextureCreativeWork
    UploadTextureFromFile(ZHLN::Vk::Allocator& allocator, const ZHLN::Vk::Context& ctx, const VkImageCreateInfo& baseInfo, const std::string& fullPath) {
    int tw = 0;
    int th = 0;
    int tc = 0;
...
auto main() -> int {
    ...
    VkPhysicalDeviceFeatures2 feat2 = {.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2, .pNext = &feat12, .features = {.samplerAnisotropy = VK_TRUE}};
    ...
    if (!ctx) {
        std::println(
            stderr, "Context creation failed. Check if your GPU supports Vulkan 1.3 and "
                    "Descriptor Indexing."
        );
        return -1;
    }
    ...
    auto stagingVBO = ZHLN::Vk::UploadToBuffer(allocator.Get(), setupCmd, vbo, scene.vertices.data(), scene.vertices.size() * sizeof(ZHLN::Vk::Vertex));
    auto stagingIBO = ZHLN::Vk::UploadToBuffer(allocator.Get(), setupCmd, ibo, scene.indices.data(), scene.indices.size() * sizeof(uint32_t));

    auto lightBuffer = ZHLN::Vk::Buffer::Create(
        allocator.Get(), sceneLights.size() * sizeof(ZHLN::Vk::Passes::Light), VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        VMA_MEMORY_USAGE_GPU_ONLY
    );

    auto stagingLights =
        ZHLN::Vk::UploadToBuffer(allocator.Get(), setupCmd, lightBuffer, sceneLights.data(), sceneLights.size() * sizeof(ZHLN::Vk::Passes::Light));
    ...
    ZHLN::Vk::TextureCreativeWork dummyTex = ZHLN::Vk::UploadTexture<VK_FORMAT_R8G8B8A8_UNORM>(allocator, ctx, dummyInfo, &white_pixel);

    uint32_t                      black_pixel = 0xFF000000; // Alpha 1.0, RGB 0.0
    ZHLN::Vk::TextureCreativeWork blackTex    = ZHLN::Vk::UploadTexture<VK_FORMAT_R8G8B8A8_UNORM>(allocator, ctx, dummyInfo, &black_pixel);
    ...
    for (cgltf_size i = 0; i < gltf->images_count; ++i) {
        ...
        if (fullPath.contains("BaseColor")) {
            textures[i] = UploadTextureFromFile<VK_FORMAT_R8G8B8A8_SRGB>(allocator, ctx, texBaseInfo, fullPath);
        } else if (fullPath.contains("Lightmap")) {
            // Lightmaps should ALWAYS be UNORM (they are data)
            textures[i] = UploadTextureFromFile<VK_FORMAT_R8G8B8A8_UNORM>(allocator, ctx, texBaseInfo, fullPath);
        } else {
    ...
    auto defaultSampler_res = ZHLN::Vk::SamplerBuilder {}.Linear().Repeat().Anisotropy(8.0f).Build(ctx.Device());
    if (!defaultSampler_res) {
        std::println(stderr, "FATAL: Failed to build default sampler: {}", defaultSampler_res.error());
        return -1;
    }
    auto defaultSampler = std::move(*defaultSampler_res);

    auto shadowSampler_res = ZHLN::Vk::SamplerBuilder {}.Linear().ClampToBorder(VK_BORDER_COLOR_FLOAT_OPAQUE_WHITE).DepthCompare().Build(ctx.Device());
    ...
    if (!lightmapSampler_res) {
        std::println(stderr, "FATAL: Failed to build lightmap sampler: {}", lightmapSampler_res.error());
        return -1;
    }
    ...
    VkDescriptorSet globalSet = SceneLayout::Allocate(ctx.Device(), descPool.Get(), descLayout.Get());

    SceneLayout::Write(
        ctx.Device(), globalSet, ZHLN::Vk::SkipWrite {}, ZHLN::Vk::SamplerWrite {defaultSampler.Get()}, ZHLN::Vk::ImageWrite {.view = shadowMap.view.Get()},
        ZHLN::Vk::SamplerWrite {shadowSampler.Get()}, ZHLN::Vk::SamplerWrite {lightmapSampler.Get()}, ZHLN::Vk::BufferWrite {.buffer = lightBuffer.Handle()}
    );
    ...
    VkDescriptorSet fxaaSet        = FXAALayout::Allocate(ctx.Device(), fxaaDescPool.Get(), fxaaDescLayout.Get());

    // --- 2. FXAA Pipeline Layout ---
    VkDescriptorSetLayout    rawFxaaLayout  = fxaaDescLayout.Get();
    ZHLN_PipelineLayoutDesc  fxaaLayoutDesc = {.set_layouts = &rawFxaaLayout, .set_layout_count = 1, .push_constants = nullptr, .push_constant_count = 0};
    ZHLN::Vk::PipelineLayout fxaaPipelineLayout(ctx.Device(), ZHLN_CreatePipelineLayout(ctx.Device(), &fxaaLayoutDesc));

    // --- 3. FXAA Shader and Pipeline ---
    auto fxaaShaders = ZHLN::Vk::ShaderStages::FromFiles(ctx.Device(), "fxaa.hlsl.VSMain.spv", "fxaa.hlsl.PSMain.spv", "VSMain", "PSMain");
    ...
    auto shaders       = ZHLN::Vk::ShaderStages::FromFiles(ctx.Device(), paths.vert_spv, paths.frag_spv, "VSMain", "PSMain");
    auto shadowShaders = ZHLN::Vk::ShaderStages::FromFiles(ctx.Device(), paths.shadow_vert_spv, paths.shadow_frag_spv, "VSMain", "PSMain");

    // Note: mainPush now needs FRAGMENT bit to read textureIndex
    VkPushConstantRange mainPush = {
        .stageFlags = VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT, .offset = 0, .size = sizeof(ZHLN::Vk::Passes::PBRPushConstants)
    };
    VkPushConstantRange shadowPush = {.stageFlags = VK_SHADER_STAGE_VERTEX_BIT, .offset = 0, .size = sizeof(Mat4)};

    VkDescriptorSetLayout   rawLayout        = descLayout.Get();
    ZHLN_PipelineLayoutDesc mainLayoutDesc   = {.set_layouts = &rawLayout, .set_layout_count = 1, .push_constants = &mainPush, .push_constant_count = 1};
    ZHLN_PipelineLayoutDesc shadowLayoutDesc = {.set_layouts = nullptr, .set_layout_count = 0, .push_constants = &shadowPush, .push_constant_count = 1};

    ZHLN::Vk::PipelineLayout pipelineLayout(ctx.Device(), ZHLN_CreatePipelineLayout(ctx.Device(), &mainLayoutDesc));
    ZHLN::Vk::PipelineLayout shadowLayout(ctx.Device(), ZHLN_CreatePipelineLayout(ctx.Device(), &shadowLayoutDesc));
    ...
    auto shadowPipelineRes =
        ZHLN::Vk::PipelineBuilder {}.Shaders(shadowShaders).Layout(shadowLayout.Get()).Vertex<ZHLN::Vk::Vertex>().DepthOnly().CullNone().Build(ctx.Device());
    ...
    while (win.running) {
        ...
        if (win.resized) {
            ...
            FXAALayout::Write(ctx.Device(), fxaaSet, ZHLN::Vk::ImageWrite {.view = sceneColor.view.Get()}, ZHLN::Vk::SamplerWrite {defaultSampler.Get()});
        ...
        Mat4 lightView = LookAt({sunPos[0], sunPos[1], sunPos[2]}, {0.f, 2.f, 0.f}, {0.f, 1.f, 0.f});
        ...
        for (const auto& dc: scene.drawCalls) {
            const auto& mat = (dc.mesh->materialIndex >= 0) ? materials[dc.mesh->materialIndex] : fallbackMat;
            pbrCalls.push_back(
        ...
        AppFrameData frameData = {
            .shadowData = {.pipeline = shadowPipelineRes->Get(), .layout = shadowLayout.Get(), .drawCalls = &pbrCalls, .scene = &sceneCtx},
            .pbrData    = {.pipeline = pipelineRes->Get(), .layout = pipelineLayout.Get(), .drawCalls = &pbrCalls, .scene = &sceneCtx},
            .fxaaData   = {.pipeline = fxaaPipelineRes->Get(), .layout = fxaaPipelineLayout.Get(), .set = fxaaSet}
        };
        ...
        ZHLN::Vk::DynamicPass({.width = SHADOW_RES, .height = SHADOW_RES}) // Compile-time deduced starting state
        ...
        ZHLN::Vk::DynamicPass(presentation.swapchain.Get().extent) // Compile-time deduced starting state
