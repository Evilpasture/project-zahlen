...
// --- Backend Math Helper ---
inline constexpr std::array<float, 16> Multiply(const std::array<float, 16>& a, const std::array<float, 16>& b) {
...
static void DrawShadows(VkCommandBuffer cmd, const ShadowConfig& c) {
    DrawBatchConfig batchCfg = {c.pipeline, c.layout, c.scene->vbo, c.scene->ibo, VK_NULL_HANDLE, VK_SHADER_STAGE_VERTEX_BIT};
...
static void DrawPBR(VkCommandBuffer cmd, const PBRMainConfig& c) {
    DrawBatchConfig batchCfg = {c.pipeline,   c.layout,           c.scene->vbo,
                                c.scene->ibo, c.scene->globalSet, VK_SHADER_STAGE_VERTEX_BIT | VK_SHADER_STAGE_FRAGMENT_BIT};
    ...
    DrawBatch<PBRPushConstants>(cmd, batchCfg, [&](auto draw) {
        for (const auto& dc: *c.drawCalls) {
            PBRPushConstants pc = {
                ...
                .lightDir         = {c.scene->lightDir[0], c.scene->lightDir[1], c.scene->lightDir[2], 0.0f},
...
static void DrawFXAA(VkCommandBuffer cmd, const FXAAConfig& c) {
    ...
    vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, c.layout, 0, 1, &c.set, 0, nullptr);
