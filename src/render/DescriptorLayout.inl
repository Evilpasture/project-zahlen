...
void BindlessRegistry<Layout, BindingID>::Init(const VkDevice device, const VkDescriptorSet set, VkImageView defaultView, VkSampler defaultSampler) {
    _device         = device;
    _set            = set;
    _nextSlot       = 0;
...
template <typename Layout, uint32_t BindingID>
auto BindlessRegistry<Layout, BindingID>::RegisterImage(const VkImageView view, const VkImageLayout layout) -> uint32_t
...
template <typename Layout, uint32_t BindingID>
auto BindlessRegistry<Layout, BindingID>::RegisterCombined(const VkImageView view, const VkSampler sampler, const VkImageLayout layout) -> uint32_t
...
template <typename Layout, uint32_t BindingID>
auto BindlessRegistry<Layout, BindingID>::UpdateDescriptor(VkImageView view, VkSampler sampler, VkImageLayout layout) -> uint32_t {
    uint32_t slot = 0;
...
inline void DescriptorUpdater::BindUniformBuffer(uint32_t binding, VkBuffer buffer, VkDeviceSize offset, VkDeviceSize range) {
    uint32_t idx      = _bufferCount++;
    auto&    buf_info = _bufferInfos[idx];
    buf_info          = {.buffer = buffer, .offset = offset, .range = range};
...
inline void DescriptorUpdater::BindStorageBuffer(uint32_t binding, VkBuffer buffer, VkDeviceSize offset, VkDeviceSize range) {
    auto& buf_info = _bufferInfos[_bufferCount++];
    buf_info       = {.buffer = buffer, .offset = offset, .range = range};
...
inline void DescriptorUpdater::BindSampledImage(uint32_t binding, VkImageView view, VkSampler sampler, VkImageLayout layout) {
    auto& img_info = _imageInfos[_imageCount++];
    img_info       = {.sampler = sampler, .imageView = view, .imageLayout = layout};
    ...
    write       = {
        ...
        .dstArrayElement  = 0,
        .descriptorCount  = 1,
        .descriptorType   = sampler == VK_NULL_HANDLE ? VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE : VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER,
        .pImageInfo       = &img_info,
        .pBufferInfo      = {},
        .pTexelBufferView = {},
    };
...
inline void DescriptorUpdater::BindSampler(uint32_t binding, VkSampler sampler) {
    auto& img_info = _imageInfos[_imageCount++];
    img_info       = {.sampler = sampler, .imageView = VK_NULL_HANDLE, .imageLayout = VK_IMAGE_LAYOUT_UNDEFINED};
...
inline void DescriptorUpdater::BindAccelerationStructure(uint32_t binding, const VkAccelerationStructureKHR* as) {
    auto& as_info = _asInfos[_asCount++];
    as_info       = {
...
constexpr auto DescriptorLayout<Slots...>::MakeBindings() noexcept {
...
template <typename... Slots>
auto DescriptorLayout<Slots...>::CreateLayout(VkDevice device) noexcept -> ZHLN::Vk::DescriptorSetLayout {
    constexpr auto bindings          = MakeBindings();
    constexpr auto flags             = MakeBindingFlags();
    constexpr bool update_after_bind = HasUpdateAfterBind();
    ...
    const VkDescriptorSetLayoutCreateInfo info = {
        ...
        .pNext        = &flags_info,
        .flags        = update_after_bind ? VK_DESCRIPTOR_SET_LAYOUT_CREATE_UPDATE_AFTER_BIND_POOL_BIT : static_cast<VkDescriptorSetLayoutCreateFlagBits>(0),
        .bindingCount = kCount,
...
template <typename... Slots>
auto DescriptorLayout<Slots...>::CreatePool(VkDevice device, uint32_t maxSets) noexcept -> ZHLN::Vk::DescriptorPool {
    const auto            pool_sizes        = MakePoolSizes(maxSets);
    static constexpr bool update_after_bind = HasUpdateAfterBind();

    const VkDescriptorPoolCreateInfo info = {
        .sType         = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO,
        .pNext         = nullptr,
        .flags         = update_after_bind ? VK_DESCRIPTOR_POOL_CREATE_UPDATE_AFTER_BIND_BIT : static_cast<VkDescriptorPoolCreateFlagBits>(0),
        .maxSets       = maxSets,
...
template <typename... Slots>
auto DescriptorLayout<Slots...>::Allocate(VkDevice device, VkDescriptorPool pool, VkDescriptorSetLayout layout) noexcept -> VkDescriptorSet {
...
void DescriptorLayout<Slots...>::Write(VkDevice device, VkDescriptorSet set, Args&&... args) noexcept {
    ...
    WriteAll(device, set, arg_tuple, image_infos, buffer_infos, as_infos, writes, std::make_index_sequence<kCount> {});
}
...
void DescriptorLayout<Slots...>::WriteAll(
    ...
    (WriteSlot<I, std::tuple_element_t<I, std::tuple<Slots...>>>(set, std::get<I>(args), imageInfos[I], bufferInfos[I], asInfos[I], writes[I]), ...);
...
void DescriptorLayout<Slots...>::WriteSlot(
    VkDescriptorSet                               set,
    const Arg&                                    arg,
    ...
    if constexpr (std::is_same_v<T, SkipWrite>) {
        static_assert(
            ((Slot::flags & VK_DESCRIPTOR_BINDING_UPDATE_AFTER_BIND_BIT) != 0) || ((Slot::flags & VK_DESCRIPTOR_BINDING_PARTIALLY_BOUND_BIT) != 0),
        ...
        if constexpr (Slot::type == VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE || Slot::type == VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER) {
        ...
        } else if constexpr (Slot::type == VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER || Slot::type == VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) {
            ...
            if constexpr (
                std::is_pointer_v<T> && (std::is_same_v<std::remove_pointer_t<T>, const VkAccelerationStructureKHR> ||
                ...
                static_assert(sizeof(T) == 0, "Unsupported argument type for AccelerationStructure slot.");
            }
        }
    }
}
...
inline void AllocateDoubleBufferedSet(
    VkDevice                               device,
    DescriptorSetLayout&                   outLayout,
    DescriptorPool&                        outPool,
    ZHLN::DoubleBuffered<VkDescriptorSet>& outSets
) noexcept {
...
template <typename LayoutT>
inline void AllocateSingleBufferedSet(VkDevice device, DescriptorSetLayout& outLayout, DescriptorPool& outPool, VkDescriptorSet& outSet) noexcept {
    outLayout = LayoutT::CreateLayout(device);
