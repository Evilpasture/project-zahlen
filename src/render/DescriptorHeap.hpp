...
using PFN_vkCmdBindSamplerHeapEXT = void(VKAPI_PTR*)(VkCommandBuffer commandBuffer,
													 const VkBindHeapInfoEXT* pBindInfo);
using PFN_vkCmdBindResourceHeapEXT = void(VKAPI_PTR*)(VkCommandBuffer commandBuffer,
													  const VkBindHeapInfoEXT* pBindInfo);
...
class DescriptorHeap {
  public:
	...
	[[nodiscard]] auto Init(const Context& ctx, Allocator& allocator, DescriptorHeapType type,
							uint32_t capacity) noexcept -> bool;
	void Cleanup() noexcept;

	void Bind(VkCommandBuffer cmd) const noexcept;

	[[nodiscard]] auto GetDevice() const noexcept -> VkDevice { return _device; }
	[[nodiscard]] auto GetBuffer() const noexcept -> VkBuffer { return _buffer.Handle(); }
	[[nodiscard]] auto GetDeviceAddress() const noexcept -> VkDeviceAddress {
		return _deviceAddress;
	}
	[[nodiscard]] auto GetStride() const noexcept -> VkDeviceSize { return _stride; }
	[[nodiscard]] auto GetCapacity() const noexcept -> uint32_t { return _capacity; }
	[[nodiscard]] auto GetType() const noexcept -> DescriptorHeapType { return _type; }
	[[nodiscard]] auto GetMappedPtr() const noexcept -> void* { return _mappedPtr; }
	[[nodiscard]] auto Valid() const noexcept -> bool { return _buffer.Valid(); }
	explicit operator bool() const noexcept { return Valid(); }

	[[nodiscard]] auto GetWriteResourceDescriptorsFn() const noexcept
		-> PFN_vkWriteResourceDescriptorsEXT {
		return _vkWriteResourceDescriptorsEXT;
	}
	[[nodiscard]] auto GetWriteSamplerDescriptorsFn() const noexcept
...
class ResourceWriteBatch {
  public:
	...
	void AddImage(uint32_t slot, const VkImageViewCreateInfo& viewInfo, VkImageLayout layout,
				  VkDescriptorType type = VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE) noexcept;

	void AddBuffer(uint32_t slot, VkDeviceAddress address, VkDeviceSize size,
				   VkDescriptorType type = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER) noexcept;

	void Flush(VkDevice device, PFN_vkWriteResourceDescriptorsEXT writeFn, void* mappedPtr,
...
class SamplerWriteBatch {
  public:
	...
	void Flush(VkDevice device, PFN_vkWriteSamplerDescriptorsEXT writeFn, void* mappedPtr,
			   VkDeviceSize stride) noexcept;
...
class HeapManager {
  public:
	...
	[[nodiscard]] auto Init(const Context& ctx, Allocator& allocator, uint32_t staticResourceCount,
							uint32_t dynamicResourceCount, uint32_t staticSamplerCount,
							uint32_t dynamicSamplerCount, uint32_t doubleBufferCount = 2) noexcept
		-> bool;
