...
static constexpr Mat4 LookAt(const std::array<float, 3>& eye, const std::array<float, 3>& center, const std::array<float, 3>& up) noexcept {
    ...
    const std::array<float, 3> s      = {f_norm[1] * up[2] - f_norm[2] * up[1], f_norm[2] * up[0] - f_norm[0] * up[2], f_norm[0] * up[1] - f_norm[1] * up[0]};
    ...
    const std::array<float, 3> u = {
        s_norm[1] * f_norm[2] - s_norm[2] * f_norm[1], s_norm[2] * f_norm[0] - s_norm[0] * f_norm[2], s_norm[0] * f_norm[1] - s_norm[1] * f_norm[0]
    };
...
static constexpr Mat4 Ortho(float left, float right, float bottom, float top, float znear, float zfar) {
    Mat4 m {};
    // X Scale
...
static_assert(Identity().data[0] == 1.0f && Identity().data[5] == 1.0f, "Identity diagonal check failed");
static_assert(Identity().data[1] == 0.0f && Identity().data[4] == 0.0f, "Storage is not contiguous or indexing is swapped");
