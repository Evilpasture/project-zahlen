...
DescriptorHeap::DescriptorHeap(DescriptorHeap&& other) noexcept
	: _device(std::exchange(other._device, VK_NULL_HANDLE)),
	  ...
	  _capacity(std::exchange(other._capacity, 0)), _stride(std::exchange(other._stride, 0)),
	  _buffer(std::move(other._buffer)), _mappedRegion(std::move(other._mappedRegion)),
...
auto DescriptorHeap::operator=(DescriptorHeap&& other) noexcept -> DescriptorHeap& {
	if (this != &other) {
		...
		_vkCmdBindResourceHeapEXT = std::exchange(other._vkCmdBindResourceHeapEXT, nullptr);
		_vkWriteSamplerDescriptorsEXT = std::exchange(other._vkWriteSamplerDescriptorsEXT, nullptr);
		_vkWriteResourceDescriptorsEXT =
			std::exchange(other._vkWriteResourceDescriptorsEXT, nullptr);
	}
	return *this;
}
...
auto DescriptorHeap::Init(const Context& ctx, Allocator& allocator, DescriptorHeapType type,
						  uint32_t capacity) noexcept -> bool {
	_device = ctx.Device();
	_type = type;
	_capacity = capacity;

	_vkCmdBindSamplerHeapEXT = reinterpret_cast<PFN_vkCmdBindSamplerHeapEXT>(
		vkGetDeviceProcAddr(_device, "vkCmdBindSamplerHeapEXT"));
	_vkCmdBindResourceHeapEXT = reinterpret_cast<PFN_vkCmdBindResourceHeapEXT>(
		vkGetDeviceProcAddr(_device, "vkCmdBindResourceHeapEXT"));
	_vkWriteSamplerDescriptorsEXT = reinterpret_cast<PFN_vkWriteSamplerDescriptorsEXT>(
		vkGetDeviceProcAddr(_device, "vkWriteSamplerDescriptorsEXT"));
	_vkWriteResourceDescriptorsEXT = reinterpret_cast<PFN_vkWriteResourceDescriptorsEXT>(
		vkGetDeviceProcAddr(_device, "vkWriteResourceDescriptorsEXT"));
	...
	if (type == DescriptorHeapType::Sampler) {
		...
		const VkDeviceSize maxSize =
			std::max(props.bufferDescriptorSize, props.imageDescriptorSize);
		const VkDeviceSize maxAlign =
			std::max(props.bufferDescriptorAlignment, props.imageDescriptorAlignment);
		_stride = AlignUp(maxSize, maxAlign);
	}

	const VkDeviceSize totalBytes = _stride * _capacity;

	_buffer = Buffer::Create(allocator.Get(), totalBytes,
...
auto ResourceWriteBatch::operator=(ResourceWriteBatch&& other) noexcept
	-> ResourceWriteBatch& = default;

void ResourceWriteBatch::AddImage(uint32_t slot, const VkImageViewCreateInfo& viewInfo,
								  VkImageLayout layout, VkDescriptorType type) noexcept {
	_impl->imageInfos.push_back({.sType = VK_STRUCTURE_TYPE_IMAGE_DESCRIPTOR_INFO_EXT,
								 .pNext = nullptr,
								 .pView = &viewInfo,
								 .layout = layout});
...
void ResourceWriteBatch::AddBuffer(uint32_t slot, VkDeviceAddress address, VkDeviceSize size,
...
void ResourceWriteBatch::Flush(VkDevice device, PFN_vkWriteResourceDescriptorsEXT writeFn,
	...
	for (uint32_t i = 0; i < totalCount; ++i) {
		...
		ranges[i] = {.address = static_cast<uint8_t*>(mappedPtr) + (_impl->slots[i] * stride),
					 .size = stride};
	}
...
auto SamplerWriteBatch::operator=(SamplerWriteBatch&& other) noexcept
	-> SamplerWriteBatch& = default;
...
auto SlotAllocator::Allocate() noexcept -> uint32_t {
	...
	std::println(stderr, "[SlotAllocator] Heap section is completely full (Capacity: {})",
				 _impl->capacity);
	std::abort();
}
...
auto HeapManager::Init(const Context& ctx, Allocator& allocator, uint32_t staticResourceCount,
					   uint32_t dynamicResourceCount, uint32_t staticSamplerCount,
					   uint32_t dynamicSamplerCount, uint32_t doubleBufferCount) noexcept -> bool {
	...
	const uint32_t totalResourceCount =
		staticResourceCount + (doubleBufferCount * dynamicResourceCount);
	const uint32_t totalSamplerCount =
		staticSamplerCount + (doubleBufferCount * dynamicSamplerCount);
...
auto HeapManager::AllocateDynamicResourceRange(uint32_t count) noexcept -> uint32_t {
	const uint32_t baseSlot = _staticResourceCount + (_currentFrameIndex * _dynamicResourceCount) +
							  _dynamicResourceAllocated;
	if (_dynamicResourceAllocated + count > _dynamicResourceCount) [[unlikely]] {
		std::println(stderr,
					 "[HeapManager] Resource Dynamic Heap overflow: allocated {} but max is {}",
					 _dynamicResourceAllocated + count, _dynamicResourceCount);
		std::abort();
	}
...
auto HeapManager::AllocateDynamicSamplerRange(uint32_t count) noexcept -> uint32_t {
	const uint32_t baseSlot = _staticSamplerCount + (_currentFrameIndex * _dynamicSamplerCount) +
							  _dynamicSamplerAllocated;
	if (_dynamicSamplerAllocated + count > _dynamicSamplerCount) [[unlikely]] {
		std::println(stderr,
					 "[HeapManager] Sampler Dynamic Heap overflow: allocated {} but max is {}",
					 _dynamicSamplerAllocated + count, _dynamicSamplerCount);
		std::abort();
	}
...
void HeapManager::FlushResourceBatch(ResourceWriteBatch& batch) noexcept {
	batch.Flush(_resourceHeap.GetDevice(), _resourceHeap.GetWriteResourceDescriptorsFn(),
				_resourceHeap.GetMappedPtr(), _resourceHeap.GetStride());
}

void HeapManager::FlushSamplerBatch(SamplerWriteBatch& batch) noexcept {
	batch.Flush(_samplerHeap.GetDevice(), _samplerHeap.GetWriteSamplerDescriptorsFn(),
				_samplerHeap.GetMappedPtr(), _samplerHeap.GetStride());
}
