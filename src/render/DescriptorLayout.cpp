...
void ReportBindlessRegistryExceeded(uint32_t bindingID, uint32_t capacity) noexcept {
    std::println(stderr, "[BindlessRegistry<{}>] FATAL: Exceeded capacity of {} slots.", bindingID, capacity);
}

DescriptorPoolBuilder::DescriptorPoolBuilder(VkDevice device) noexcept: _device(device) {
}

auto DescriptorPoolBuilder::Flags(VkDescriptorPoolCreateFlags flags) noexcept -> DescriptorPoolBuilder& {
    _flags = flags;
    return *this;
}
...
auto DescriptorPoolBuilder::AddSize(VkDescriptorType type, uint32_t count) noexcept -> DescriptorPoolBuilder& {
