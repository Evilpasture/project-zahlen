...
static JPH::Array<Entity> s_VisibleEntities;
...
void UpdateEditorCamera(Camera& cam, const InputContext& input, float dt) {
    ...
    JPH::Vec3 forward(
        JPH::Cos(yawRad) * JPH::Cos(pitchRad),
        JPH::Sin(pitchRad),
    ...
    if (input.IsKeyDown(KeyCode::W)) moveDirection += forward;
    if (input.IsKeyDown(KeyCode::S)) moveDirection -= forward;
    if (input.IsKeyDown(KeyCode::A)) moveDirection -= right;
    if (input.IsKeyDown(KeyCode::D)) moveDirection += right;
...
void DrawEditorPanels(Engine& engine) {
    ...
    if (g_EditorState.selectedEntity != NullEntity && reg.IsAlive(g_EditorState.selectedEntity)) {
        ...
        ImGui::Text("Active Entity ID: %u", e.index);
        ImGui::Separator();
        ...
        if (auto* phys = reg.Get<PhysicsComponent>(e)) {
            if (ImGui::CollapsingHeader("Physics Component", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                if (!bid.IsInvalid()) {
                    ...
                    float posArr[3] = { (float)p.GetX(), (float)p.GetY(), (float)p.GetZ() };
                    float rotArr[3] = { euler.GetX(), euler.GetY(), euler.GetZ() };

                    bool pMod = ImGui::DragFloat3("Position", posArr, 0.05f);
                    bool rMod = ImGui::DragFloat3("Rotation", rotArr, 0.2f);

                    if (pMod || rMod) {
                        JPH::RVec3 nextPos(posArr[0], posArr[1], posArr[2]);
                        JPH::Quat nextRot = Math::EulerDegreesToQuat(JPH::Vec3(rotArr[0], rotArr[1], rotArr[2]));
                        world.bodyInterface->SetPositionAndRotation(bid, nextPos, nextRot, JPH::EActivation::Activate);
                    }
                }
            }
        }
        ...
        if (auto* alife = reg.Get<ALife::ALifeComponent>(e)) {
            if (ImGui::CollapsingHeader("ALife Component", ImGuiTreeNodeFlags_DefaultOpen)) {
                ...
                ImGui::Text("Current State: %s", (alife->state == ALife::State::Dead) ? "Dead" : (alife->state == ALife::State::Online) ? "Online" : "Offline");
            }
        }
    } else {
        ImGui::Text("No entity selected. Left click elements in viewport to select.");
    }
    ImGui::End();
}
...
int main([[maybe_unused]] int argc, [[maybe_unused]] char* argv[]) {
    ...
    // Spawn a temporary workspace terrain and basic landmarks (Copying Setup from your old main.cpp)
    ...
    Mesh terrainMesh = AssetFactory::CreateTerrain(rc, terrainSize, terrainWorldSize, terrainMaxHeight, terrainHeights);
    auto terrainShape = Physics::CreateHeightFieldShape(terrainHeights, terrainSize, terrainWorldSize);
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        reg.Add(box, MeshComponent{.mesh = AssetFactory::CreateBox(rc, {2,2,2}), .material = material, .cullRadius = 10.f});
        reg.Add(box, PhysicsComponent{Physics::CreateRigidBody(pc, boxShape, {(float)i * 6.0f, 15.0f, 0.0f}, JPH::Quat::sIdentity(), JPH::EMotionType::Dynamic, 1)});
    }
    ...
    while (engine.IsRunning()) {
        ...
        if (engine.GetInput().IsKeyDown(KeyCode::Unknown) == 0 && 
            engine.GetInput().IsMouseButtonDown(KeyCode::RButton) == 0 && 
        ...
        if (res.width > 0 && res.height > 0) {
            ...
            if (g_TAAState.enabled) {
                vp = cam.GetJitteredProjectionMatrix((float)res.width / res.height, res.width, res.height) * cam.GetViewMatrix();
            }
            ...
            JPH::Mat44 lightView = Math::CreateLookAt(sunDirection * 100.0f, {0.0f, 0.0f, 0.0f}, JPH::Vec3::sAxisY());
            ...
            ZHLN_LOCK(worldState.sync.shadowLock) {
                for (Entity e : s_VisibleEntities) {
                    ...
                    if (phys != nullptr) {
                        ...
                        JPH::Vec3 pos((float)worldState.positions[static_cast<size_t>(dense * 4)],
                                      (float)worldState.positions[(dense * 4) + 1],
                                      (float)worldState.positions[(dense * 4) + 2]);
                        JPH::Quat rot(worldState.rotations[static_cast<size_t>(dense * 4)],
                                      worldState.rotations[(dense * 4) + 1],
                                      worldState.rotations[(dense * 4) + 2],
                                      worldState.rotations[(dense * 4) + 3]);
                        currentTransform = Math::CreateTransform(pos, rot);
                    } else if (auto* alifeComp = reg.Get<ALife::ALifeComponent>(e)) {
                        currentTransform = Math::CreateTransform(JPH::Vec3(alifeComp->position), JPH::Quat::sIdentity());
                    } else {
                        continue;
                    }
