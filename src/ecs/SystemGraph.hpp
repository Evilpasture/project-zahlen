...
template <typename T> constexpr ComponentAccess Read() noexcept {
	return {ComponentFamily::GetTypeID<T>(), Access::Read};
}

template <typename T> constexpr ComponentAccess Write() noexcept {
...
class ZHLN_API SystemGraph {
  ...
  private:
	...
	[[nodiscard]] bool HasConflict(const SystemInfo& systemA,
