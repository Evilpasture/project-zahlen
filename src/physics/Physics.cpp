...
template <typename T>
static void ReallocateStandard(T*& ptr, size_t old_count, size_t new_count) {
    static_assert(std::is_trivially_copyable_v<T>);
...
void PhysicsContext::Step(float deltaTime) {
    ...
    updateSettings.mWalkStairsStepUp     = JPH::Vec3(0.0f, 0.5f, 0.0f);  // Steps over obstacles up to 0.5m
    updateSettings.mStickToFloorStepDown = JPH::Vec3(0.0f, -0.5f, 0.0f); // Snaps smoothly going down steps up to 0.5m
...
JPH::ShapeRefC GetOrCreateShape(PhysicsContext& ctx, Physics::ShapeType type, float p1, float p2, float p3, float p4) {
...
ZHLN::Entity CreateMeshBody(
    ...
    return CreateRigidBody(ctx, shape, pos, rot, JPH::EMotionType::Static, Layers::NON_MOVING, 0, category, mask);
}

ZHLN::Entity CreateCharacter(PhysicsContext& ctx, JPH::RVec3Arg position, uint32_t category, uint32_t mask) {
    auto* impl  = ctx.GetImpl();
...
DebugDrawData GetDebugDrawData(PhysicsContext& ctx, bool drawShapes, bool drawConstraints, bool wireframe) {
    ...
    return {
        .lines         = reinterpret_cast<const DebugVertex*>(impl->debugRenderer->lines.data()),
        .lineCount     = impl->debugRenderer->lines.size(),
        .triangles     = reinterpret_cast<const DebugVertex*>(impl->debugRenderer->triangles.data()),
...
ConstraintHandle CreateConstraint(PhysicsContext& ctx, ConstraintType type, ZHLN::Entity b1, ZHLN::Entity b2, const ConstraintParams& params) {
    auto& world = ctx.GetImpl()->world;
