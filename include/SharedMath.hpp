...
struct float3 {
    ...
    constexpr float3 operator*(float s) const {
        return {x * s, y * s, z * s};
    }
    constexpr float3 operator/(float s) const {
        return {x / s, y / s, z / s};
...
struct float4 {
    ...
    constexpr float4 operator/(float4 o) const {
        return {x / o.x, y / o.y, z / o.w, w / o.w};
    }
    constexpr float4 operator*(float s) const {
...
inline float3 clamp(float3 v, float3 minVal, float3 maxVal) {
    return {clamp(v.x, minVal.x, maxVal.x), clamp(v.y, minVal.y, maxVal.y), clamp(v.z, minVal.z, maxVal.z)};
}
