...
class SkipList {
  public:
    ...
    explicit SkipList(Compare comp = Compare()): _head(CreateNode(Key {}, Value {}, MAX_LEVEL)), _compare(comp) {
    ...
    bool Erase(const Key& key) {
        for (;;) {
            ...
            for (uint32_t i = 0; i < doomed->height; ++i) {
                ...
                if (pred->deleted.load(std::memory_order::acquire) || pred->GetForward()[i].load(std::memory_order::relaxed) != doomed) {
                    valid = false;
                    break;
                }
            }
            ...
            while (lvl > 1 && _head->GetForward()[lvl - 1].load(std::memory_order::relaxed) == nullptr) {
                lvl--;
            }
  ...
  private:
    ...
    static SkipNode* CreateNode(const Key& key, const Value& value, uint32_t height) {
        ...
        auto*  node = ::new (mem) SkipNode {.key = key, .value = value, .height = height, .deleted = {}, .lock = {}};
