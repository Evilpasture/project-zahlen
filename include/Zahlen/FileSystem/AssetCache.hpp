...
//
// A generic cache that only knows identity, lifetime and caching. It does NOT
// know how to load, parse or instantiate T. That is the job of the factory
// that produces T and then hands ownership to the cache.
//
// This belongs to the Filesystem / Asset Manager layer, not Core. Core is
// foundational low-level primitives (Hash, Span, Platform). Caching of
// high-level objects (ModelPrefab, BakedFontAsset) lives here.
//
// Thread-safe: every public operation locks internally.
// Ownership is via std::unique_ptr<T> (R.11/R.20), no explicit new/delete.
// The HashMap holds non-owning raw pointers for O(1) lookup; the vector of
// unique_ptr owns.
