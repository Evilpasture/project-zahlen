...
# ===================================================================
# CUSTOM RIG ADJUSTMENTS & TUNING (N DISASSEMBLY DRONE)
# ===================================================================
LEG_STANCE_SPREAD = 0.055

FINGER_FLEX_RANGE = 0.08
ARM_SWAY_RANGE = 2.5
WRIST_SWAY_RANGE = 4.0

# Visor Eyelid Movement Offsets
EYELID_TRAVEL_TOP = -0.035
EYELID_TRAVEL_BOT = 0.035
EYE_SACCADE_SCALE = 0.005

# Game Export Action Whitelist
VALID_ACTION_NAMES = {
    "N_IDLE",
    "N_WALKING",
    "N_RUNNING",
...
# ===================================================================
# SPARSE KEYFRAME DETERMINATION ENGINE
# ===================================================================
def is_sparse_keyframe(state_name: str, frame: int, duration: int) -> bool:
    """Determines whether a frame is a major key pose, keeping the Dope Sheet sparse and human-editable."""
    if frame == 1 or frame == duration:
        return True

    if state_name == "RUNNING":
        return frame in (1, 7, 13, 19, 24)
    elif state_name == "WALKING":
        return frame in (1, 7, 13, 19, 25, 31, 36)
    elif state_name == "STRAFE":
        return frame in (1, 9, 17, 25, 33, 40)
    elif state_name == "IDLE":
        return (frame - 1) % 15 == 0 or frame == 120
    elif state_name == "JUMPING":
        return frame in (1, 8, 15, 23, 30)
    elif state_name == "LANDING":
        return frame in (1, 7, 14, 20)

    return frame % 6 == 0


def set_action_bezier_handles(action):
    """Converts all F-Curves to smooth, editable Bezier curves (Blender 5.2 Slotted Action Safe)."""
    if not action:
        return

    fcurves = []

    # 1. Legacy / Standard Direct F-Curves
    if hasattr(action, "fcurves"):
        fcurves.extend(list(action.fcurves))

    # 2. Blender 5.2 LTS Slotted Actions API (fcurves inside slots)
    if hasattr(action, "slots"):
        for slot in action.slots:
            if hasattr(slot, "fcurves"):
                fcurves.extend(list(slot.fcurves))

    # 3. Direct Curves Array Fallback
...
# ===================================================================
# N DISASSEMBLY DRONE RIG SCHEMA
# ===================================================================
class NRigSchema:
    ...
    UP_ARM_L, FORE_ARM_L, HAND_L = "FK-Upper_arm.L", "FK-Forearm.L", "FK-Hand.L"
    UP_ARM_R, FORE_ARM_R, HAND_R = "FK-Upper_arm.R", "FK-Forearm.R", "FK-Hand.R"
    THIGH_L, SHIN_L, FOOT_L = "FK-Thigh.L", "FK-Shin.L", "FK-Foot.L"
    THIGH_R, SHIN_R, FOOT_R = "FK-Thigh.R", "FK-Shin.R", "FK-Foot.R"

    UI_WEAPONS = "CTR-Disassembly_Drone"
    WING_DEPLOY = "CTR-Wings_Deploy"
    TAIL_CHAIN = [f"CTR-Tail_{i}" for i in range(1, 7)]

    EYELID_TOP_L, EYELID_TOP_R = "CTR-Top_Eyelid.L", "CTR-Top_Eyelid.R"
    EYELID_BOT_L, EYELID_BOT_R = "CTR-Bottom_Eyelid.L", "CTR-Bottom_Eyelid.R"
    EYE_L, EYE_R = "CTR-Eye.L", "CTR-Eye.R"
...
class BoneData:
    ...
    def __init__(self, pose_bone):
        ...
        self.base_scale = (
            pose_bone.scale.copy() if pose_bone else Vector((1.0, 1.0, 1.0))
        )
    ...
    @property
    def scale(self):
        return self.bone.scale if self.bone else Vector((1.0, 1.0, 1.0))
...
class CharacterRigN:
    ...
    def __init__(self, body_rig, face_rig, is_locomotion=False):
        self.rig = body_rig
        self.face_rig = face_rig
        self.pose_bones = body_rig.pose.bones
        face_bones = face_rig.pose.bones if face_rig else self.pose_bones

        self.torso = BoneData(self.pose_bones.get(NRigSchema.TORSO))
        self.chest = BoneData(self.pose_bones.get(NRigSchema.CHEST))
        self.hips = BoneData(self.pose_bones.get(NRigSchema.HIPS))
        self.head = BoneData(self.pose_bones.get(NRigSchema.HEAD))

        self.up_arm_l = BoneData(self.pose_bones.get(NRigSchema.UP_ARM_L))
        self.fore_arm_l = BoneData(self.pose_bones.get(NRigSchema.FORE_ARM_L))
        self.hand_l = BoneData(self.pose_bones.get(NRigSchema.HAND_L))
        self.up_arm_r = BoneData(self.pose_bones.get(NRigSchema.UP_ARM_R))
        self.fore_arm_r = BoneData(self.pose_bones.get(NRigSchema.FORE_ARM_R))
        self.hand_r = BoneData(self.pose_bones.get(NRigSchema.HAND_R))

        foot_l_bone = self.pose_bones.get("FK-Foot.L") or self.pose_bones.get(
            "CTR-Foot.L"
        )
        foot_r_bone = self.pose_bones.get("FK-Foot.R") or self.pose_bones.get(
            "CTR-Foot.R"
        )

        self.thigh_l = BoneData(self.pose_bones.get(NRigSchema.THIGH_L))
        self.shin_l = BoneData(self.pose_bones.get(NRigSchema.SHIN_L))
        self.foot_l = BoneData(foot_l_bone)
        self.thigh_r = BoneData(self.pose_bones.get(NRigSchema.THIGH_R))
        self.shin_r = BoneData(self.pose_bones.get(NRigSchema.SHIN_R))
        self.foot_r = BoneData(foot_r_bone)

        self.wing_deploy = BoneData(self.pose_bones.get(NRigSchema.WING_DEPLOY))
        self.tail_chain = [
            BoneData(self.pose_bones.get(b_name)) for b_name in NRigSchema.TAIL_CHAIN
        ]

        # Unlock Tail Constraints for Smooth FK Rotation
        for bd in self.tail_chain:
            if bd.bone:
                bd.bone.rotation_mode = "XYZ"
                for c in bd.bone.constraints:
                    c.mute = True
                    c.influence = 0.0

        self.weapons_ui = self.pose_bones.get(NRigSchema.UI_WEAPONS)
        self.eyelid_top_l = BoneData(face_bones.get(NRigSchema.EYELID_TOP_L))
        self.eyelid_top_r = BoneData(face_bones.get(NRigSchema.EYELID_TOP_R))
        self.eyelid_bot_l = BoneData(face_bones.get(NRigSchema.EYELID_BOT_L))
        self.eyelid_bot_r = BoneData(face_bones.get(NRigSchema.EYELID_BOT_R))

        self.eye_l_bone = face_bones.get(NRigSchema.EYE_L)
        self.eye_r_bone = face_bones.get(NRigSchema.EYE_R)
        self.eye_l = BoneData(self.eye_l_bone)
        self.eye_r = BoneData(self.eye_r_bone)
    ...
    def setup_default_disassembly_state(self, frame=1):
        if self.wing_deploy.bone:
            self.wing_deploy.scale = Vector((0.0, 0.0, 0.0))
            self.wing_deploy.keyframe_insert("scale", frame=frame)

        if self.weapons_ui:
            for side in ["L", "R"]:
                for weapon_prop in ["Claw", "Blade", "SMG"]:
                    prop_key = f"{weapon_prop}.{side}"
                    if prop_key in self.weapons_ui:
                        self.weapons_ui[prop_key] = 0.0
                        self.weapons_ui.keyframe_insert(
                            data_path=f'["{prop_key}"]', frame=frame
                        )

                hand_key = f"Hand.{side}"
                if hand_key in self.weapons_ui:
                    self.weapons_ui[hand_key] = 1.0
                    self.weapons_ui.keyframe_insert(
                        data_path=f'["{hand_key}"]', frame=frame
                    )

        for eye_b in [self.eye_l_bone, self.eye_r_bone]:
            if eye_b and "Eye_Shape" in eye_b:
                eye_b["Eye_Shape"] = 1.0
                eye_b.keyframe_insert(data_path='["Eye_Shape"]', frame=frame)
...
MOTION_PROFILES = {
    ...
    "WALKING": {
        ...
        "legs": {"style": "stride", "amplitude": 0.55},
    ...
    "RUNNING": {
        ...
        "arms": {"style": "pump", "sway": 0.8, "delay": 0.0, "flare": 8.0},
        "fingers": {"curl": 0.65},
        "eyes": {"blink": False, "saccade": 0.1},
        "legs": {"style": "stride", "amplitude": 0.78},
    ...
    "STRAFE": {
        ...
        "legs": {"style": "sidestep", "amplitude": 0.38},
...
def _prepare_rig_for_animation(rig, face_rig):
    props = rig.data
    for prop_name in NRigSchema.IK_FK_PROPS:
...
def _keyframe_legs(
    ...
    if rig.foot_l.bone:
        rig.foot_l.location = rig.foot_l.base_loc + Vector(
            (-LEG_STANCE_SPREAD, 0.0, 0.0)
        )
    ...
    if rig.foot_r.bone:
        rig.foot_r.location = rig.foot_r.base_loc + Vector(
            (LEG_STANCE_SPREAD, 0.0, 0.0)
        )
...
def _keyframe_arms_idle(
    ...
    if rig.fore_arm_l.bone:
        elbow_offset = (
            get_organic_breath_curve(breath_phase - 0.7) * arm_sway_rad
        )
    ...
    if rig.hand_l.bone:
        wrist_offset = (
            get_organic_breath_curve(breath_phase - 1.0) * wrist_sway_rad
        )
...
def _keyframe_arms_locomotion(
    ...
    if rig.fore_arm_r.bone:
        r_elbow = (
            base_elbow + math.sin(walk_phase - 0.5 - math.pi) * elbow_flex_amp
        )
        rig.fore_arm_r.rotation_euler = Euler(
            (
...
# ===================================================================
# TAIL KEYFRAMING ON SPARSE KEYFRAMES
# ===================================================================
def _keyframe_tail_sparse(rig_data, state_name, phase, progress, frame):
    """Calculates smooth connected FK tail poses ONLY on sparse keyframes."""
    for idx, bd in enumerate(rig_data.tail_chain):
        if not bd.bone:
            continue

        delay = idx * 0.45
        p_delayed = phase - delay

        # Keep child joint origins connected at sockets (location = base_loc)
        bd.location = bd.base_loc

        pitch = 0.0
        yaw = 0.0
        roll = 0.0

        if state_name == "IDLE":  # Compact Scorpion Loop
            if idx == 0:
            ...
            yaw = math.cos(p_delayed * 0.5) * math.radians(12.0)

        elif state_name == "RUNNING":  # Aerodynamic Trailing Wave
            if idx == 0:
                pitch = math.radians(-35.0)
            else:
                pitch = math.radians(18.0) + math.sin(p_delayed * 2.0) * math.radians(15.0)
            yaw = math.sin(p_delayed) * math.radians(14.0)

        elif state_name == "WALKING":  # Step Counter-Balance Sway
            if idx == 0:
                pitch = math.radians(-40.0)
            else:
                pitch = math.radians(15.0) + math.sin(p_delayed * 2.0) * math.radians(10.0)
            yaw = math.sin(p_delayed) * math.radians(18.0)

        elif state_name == "STRAFE":  # Lateral Balance
            if idx == 0:
                pitch = math.radians(-45.0)
            else:
                pitch = math.radians(20.0)
            yaw = -math.sin(p_delayed) * math.radians(22.0)

        elif state_name == "JUMPING":  # Skyward Flight
            if progress < 0.25:
                pitch = math.radians(-60.0)
            elif progress < 0.48:
                pitch = math.radians(30.0)
        ...
        elif state_name == "LANDING":  # Impact Forward Whip
            if progress < 0.35:
                t = progress / 0.35
                pitch = math.radians(60.0) * math.sin(t * math.pi / 2.0)
            else:
                t = (progress - 0.35) / 0.65
                interp = (1.0 - math.cos(t * math.pi)) / 2.0
                pitch = math.radians(60.0) * (1.0 - interp) - math.radians(40.0) * interp

        bd.rotation_euler = Euler((bd.base_rot.x + pitch, bd.base_rot.y + roll, bd.base_rot.z + yaw))

        bd.keyframe_insert("location", frame=frame)
        bd.keyframe_insert("rotation_euler", frame=frame)


# ===================================================================
# UNIVERSAL GENERATOR ENGINE FOR N (SPARSE KEY PASS)
# ===================================================================
def generate_N_state(body_rig, face_rig, state_name: str, duration: int):
    ...
    rig_data = CharacterRigN(body_rig, face_rig, is_locomotion=is_loco)
    rig_data.setup_default_disassembly_state(frame=1)

    bpy.context.scene.frame_start = 1
    bpy.context.scene.frame_end = duration

    for frame in range(1, duration + 1):
        # ONLY KEYFRAME ON SPARSE BREAKDOWN FRAMES!
        if not is_sparse_keyframe(state_name, frame, duration):
            continue
        ...
        if profile.torso_style == "jump":
            if progress < 0.25:
                ...
                t_sin = math.sin(t * math.pi / 2.0)
                torso_z = -0.10 + (0.18 - (-0.10)) * t_sin
                torso_y = -0.04 * (1.0 - t_sin)
                torso_pitch = (
                ...
                t_sin = math.sin(t * math.pi / 2.0)
                torso_z = 0.18 - 0.16 * t_sin
                torso_y = 0.0
                torso_pitch = math.radians(-6.0) * (1.0 - t_sin) + math.radians(
                    8.0
                ) * t_sin
                knee_bend = math.radians(18.0) * t_sin
                thigh_swing = math.radians(-12.0) * (1.0 - t_sin) + math.radians(
                    10.0
                ) * t_sin
                foot_flex = math.radians(-28.0) * (1.0 - t_sin) + math.radians(
                    10.0
                ) * t_sin
                arm_swing = math.radians(48.0) * (1.0 - t_sin) + math.radians(
                    12.0
                ) * t_sin
            else:
            ...
            if progress < 0.35:
                ...
                t = (progress - 0.35) / 0.65
                interp = (1.0 - math.cos(t * math.pi)) / 2.0
                torso_z = -0.15 * (1.0 - interp)
                torso_y = -0.06 * (1.0 - interp)
        ...
        if profile.leg_style in ("jump", "land"):
            ...
            l_knee_bend = math.radians(15.0) + max(
                0.0, math.sin(phase - 0.7)
            ) * (amp * 1.0)
            r_knee_bend = math.radians(15.0) + max(
                0.0, math.sin(phase + math.pi - 0.7)
            ) * (amp * 1.0)
        ...
        # Secondary Facial, Finger, and Sparse Tail Dispatch
        _keyframe_fingers(rig_data, phase, profile.finger_curl, frame=frame)
        _keyframe_tail_sparse(rig_data, state_name, phase, progress, frame=frame)
    ...
    # POST-PASS: Convert all generated F-Curves to smooth, editable BEZIER handles!
    if body_rig.animation_data and body_rig.animation_data.action:
        set_action_bezier_handles(body_rig.animation_data.action)
    if face_rig and face_rig.animation_data and face_rig.animation_data.action:
        set_action_bezier_handles(face_rig.animation_data.action)
...
# ===================================================================
# ACTION OVERWRITE & BATCH EXPORTER
# ===================================================================
...
def generate_all_N_actions():
    ...
    if not body_rig or body_rig.type != "ARMATURE":
        print("Please select N's main Armature ('Rig') in the 3D Viewport.")
        return
    ...
    for state_type, duration in states.items():
        ...
        print(f"Generating Sparse Human-Editable Action: '{action_name}'...")
        generate_N_state(body_rig, face_rig, state_type, duration)

    # Preview N_RUNNING
    run_action = bpy.data.actions.get("N_RUNNING")
    if run_action:
        body_rig.animation_data.action = run_action
        if face_rig:
            face_rig.animation_data.action = run_action
    ...
    print("Sparse Keyframe Generation Complete! Keyframes are spaced 6–15 frames apart with smooth Bezier handles.")


# Execute Generator
generate_all_N_actions()