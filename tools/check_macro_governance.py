...
2. EXPANSIONS (opt-in, `--full`, needs the libclang Python bindings)
   ...
   This pass checks that any macro defined within the repository is allowlisted.
   Macros originating from outside the repository (system headers, libc/libc++,
   compiler built-ins, and vendored code) are automatically allowed.
...
def check_banned(paths, banned, violations) -> None:
    ...
    for name, spec in banned.items():
        function_like = (
            bool(spec.get("function_like", False)) if isinstance(spec, dict) else False
        )
        pattern = re.compile(
            rf"\b{re.escape(name)}\s*\(" if function_like else rf"\b{re.escape(name)}\b"
        )
        for path in iter_sources(paths):
            if is_vendored(path):
                continue
...
INCLUDE_DIRS = (
    ...
    "extern/stb",
    "extern/miniaudio",
    "extern/mimalloc/include",
    "extern/meshoptimizer",
    "extern/LuaJIT/src",
)
...
def _load_shared_library(cindex) -> bool:
    ...
    for base in candidates:
        roots = (
            [base]
            if base.is_file()
            else sorted(base.rglob("libclang*.so*"))
            if base.is_dir()
            else []
        )
        for cand in roots:
...
def _defined_in_repo(cursor) -> bool:
    ...
    Returns False for platform, system, compiler built-in, and vendored macros,
    which are not ours to govern.
    """
    definition = cursor.get_definition()
    # If libclang cannot resolve the definition file, it is a built-in,
    # a command-line flag (-D), or an unparsed external system macro.
...
def check_expansions_libclang(
    ...
    if env_flags:
        flags += env_flags
        print(
            f"  using {len(env_flags)} flags from ZHLN_LIBCLANG_FLAGS", file=sys.stderr
        )
    scanned = 0
    for path in iter_sources(paths):
        if is_vendored(path):
        ...
        try:
            ...
            if path.suffix in TU_SUFFIXES:
                ...
                print(
                    f"  note: could not parse {path.name} standalone; skipped",
                    file=sys.stderr,
                )
            continue
        ...
        for cursor in tu.cursor.get_children():
            ...
            if cursor.kind == cindex.CursorKind.MACRO_DEFINITION:
                ...
                if name in banned:
                    # Banned macros are prohibited regardless of provenance
                    violations.append(
                    ...
                    if name not in allowed and not any(
                        ...
                        if name.startswith("__") or (
                            name.startswith("_") and name[:2].isupper()
                        ):
                            continue
                        violations.append(
        ...
        if records == 0:
            ...
            if why is None:
                print(
                    f"  note: {path.name} yielded no macro records "
                    f"(module units are not fully supported by libclang); "
                    f"covered by the definitions pass",
                    file=sys.stderr,
                )
                continue
            if path.suffix in TU_SUFFIXES:
                ...
                print(
                    f"  note: {path.name} is not a standalone TU; covered by the "
                    f"definitions pass",
                    file=sys.stderr,
                )
            continue
...
def main() -> int:
    ...
    print(
        f"Macro governance OK ({scope}, {len(allowed)} allowlisted, {len(banned)} banned{extra})."
    )
