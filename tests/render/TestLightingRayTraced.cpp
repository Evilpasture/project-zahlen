...
//      must not blow out or produce black holes (artifacts), and the RTR
//      path must not degenerate to the IBL fallback when compared with SSR.
//
...
enum class LightingRTTestError : uint8_t {
    ...
    ReflectionArtifacts[[= ZHLN::Reflect::Description("The reflected region contains blowout, black holes, or flicker.")]],
...
struct FrameMetrics {
    ...
    uint32_t black        = 0; // r+g+b <= 6 (holes / dead pixels)
    uint32_t red          = 0; // r >= 60 and clearly red-dominant (light/reflection signature)
    uint32_t redPeak      = 0; // brightest red-dominant pixel
    uint32_t redIsolated  = 0; // red pixels with 3+ of 4 neighbors black: speckles/ray debris
    double   meanLuma     = 0.0;
    double   maxLuma      = 0.0;
};
...
[[nodiscard]] FrameMetrics MeasureImage(const RgbImage& img, double minRowFraction = 0.0) {
    ...
    for (size_t i = 0; i < img.rgb.size(); i += 3) {
        ...
        if (r >= 60 && r >= 1.6 * static_cast<double>(g) && r >= 1.6 * static_cast<double>(b)) {
            ...
            blackNeighbors += isBlackAt(x - 1, y) ? 1u : 0u;
            blackNeighbors += isBlackAt(x + 1, y) ? 1u : 0u;
            blackNeighbors += isBlackAt(x, y - 1) ? 1u : 0u;
            blackNeighbors += isBlackAt(x, y + 1) ? 1u : 0u;
            if (blackNeighbors >= 3) {
                ++m.redIsolated;
            }
...
struct LightingRTTestSuite {
    ...
    static auto Capture(ZHLN::RenderContext& rc, const std::string& path) -> RgbImage {
    ...
    struct Tests {
        ...
        std::expected<void, ZHLN::Error> lit_scene_static_frame_stability() {
            ...
            // Warm the renderer (Hi-Z history, TLAS, cluster buffers).
            ...
            const RgbImage repeatA = Capture(rc, "headless_lighting_rt_static_r0.ppm");
            const RgbImage repeatB = Capture(rc, "headless_lighting_rt_static_r1.ppm");
            ...
            for (uint32_t f = 0; f < 4; ++f) {
                TickFrames(*engine, 1);
                const RgbImage frame = Capture(rc, "headless_lighting_rt_static_f" + std::to_string(f) + ".ppm");
            ...
            ZHLN::Test::ExpectFalse(engine->GetVisibleEntities().empty());
            ...
            if (!stableLit || !stableLuma || !stableRed || !stableSat || !noPixelPop || !noJump || !noBlowout) {
        ...
        std::expected<void, ZHLN::Error> point_light_cluster_culling_sweep() {
            ...
            for (int step = 0; step < kSteps; ++step) {
                ...
                const float z = 5.0f - 0.078125f * x;
                ...
                const RgbImage frame = Capture(rc, "headless_lighting_rt_cull_" + std::to_string(step) + ".ppm");
                auto checkFrame = ZHLN::Test::AssertTrue(frame.Valid());
                if (!checkFrame) {
                    return std::unexpected(LightingRTTestError::RenderOutputBlank);
                }
            ...
            if (!neverCulled || !brightEverywhere || !noSuddenSlump) {
        ...
        std::expected<void, ZHLN::Error> raytraced_shadow_occlusion_and_stability() {
            ...
            // Thin tall occluder wall: 6m tall, 8m deep, 1m thick. Its shadow
            // extends x in [-8.5, -0.5] on the floor, wholly in the camera's
            // foreground. The wall's own unlit face stays above the measured
            // rows, so the metrics isolate the cast shadow.
            const ZHLN::Entity occluder = ZHLN::CreativeWorksFactory::CreateBox(
                *engine, JPH::Vec3(0.5f, 3.0f, 4.0f),
                ZHLN::CreativeWorksFactory::SpawnParams {.position = JPH::RVec3(0.0, 3.0, -8.0), .createPhysics = false, .color = {0.7f, 0.7f, 0.7f, 1.0f}}
            );
            ...
            const RgbImage shadowA       = Capture(rc, "headless_lighting_rt_shadow_a.ppm");
            const RgbImage shadowARepeat = Capture(rc, "headless_lighting_rt_shadow_a_repeat.ppm");
            TickFrames(*engine, 1);
            const RgbImage shadowB = Capture(rc, "headless_lighting_rt_shadow_b.ppm");
            ...
            TickFrames(*engine, 2);
            const RgbImage shadowClear = Capture(rc, "headless_lighting_rt_shadow_clear.ppm");
            ...
            // Relative-darkness separation: sunlit floor is far brighter than
            // the ambient-only shadowed floor at exposure 1.5.
            ...
            if (!shadowExist || !lightRestored || !notBlackout || !clearNotDark || !shadowStable || !noShadowFlicker || !repeatClean) {
        ...
        std::expected<void, ZHLN::Error> raytraced_reflection_coverage_and_artifacts() {
            ...
            for (uint32_t f = 0; f < 4; ++f) {
                TickFrames(*engine, 1);
                const RgbImage frame = Capture(rc, "headless_lighting_rt_reflect_f" + std::to_string(f) + ".ppm");
                auto checkFrame = ZHLN::Test::AssertTrue(frame.Valid());
                if (!checkFrame) {
                    return std::unexpected(LightingRTTestError::RenderOutputBlank);
                }
                ...
                isolatedSeries.push_back(
                    m.red > 0 ? static_cast<double>(m.redIsolated) / static_cast<double>(m.red) : 0.0
                );
            }
            ...
            const bool noBlowout =
                ZHLN::Test::ExpectTrue(meanSaturation < 0.04 * static_cast<double>(lowerHalfPixels));
            ...
            if (rc.RayTracingSupported()) {
                auto setReflectionPath = [&](int enableSSR, int enableRTR, const std::string& tag) -> double {
                    ...
                    TickFrames(*engine, 8);
                    const RgbImage frame = Capture(rc, "headless_lighting_rt_reflect_" + tag + ".ppm");
                    if (!frame.Valid()) {
                        return -1.0;
                    }
                ...
                if (!rtrHasReflection || !ssrHasReflection || !rtrNotDegraded || !ssrNotDegraded) {
                    return std::unexpected(LightingRTTestError::ReflectionMissing);
                }
            }
