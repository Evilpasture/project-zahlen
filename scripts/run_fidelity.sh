...

{
    ...
    echo
    echo "rule render"
    echo "  command = env ZHLN_CACHE_DIR=$SHARED_CACHE $ENGINE_BIN --headless --scenario=$OUT_DIR/\$scenario.json --output=$OUT_DIR/\$scenario.pam"
    echo "  description = render \$scenario"
    echo "rule compare"
    echo "  command = $TOOL_BIN compare --candidate $OUT_DIR/\$scenario.pam --goldens-dir $FIDELITY/test/goldens --name \$scenario --out-dir $OUT_DIR"
