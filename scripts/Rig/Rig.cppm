...
struct JointDesc {
    ...
    Limb             limbGroup;
    float            bindPosition[3]; // Raw floats are constexpr-safe
    float            bindRotation[4]; // Raw floats are constexpr-safe
};
...
[[nodiscard]] inline JPH::Vec3 GetBindPosition(Joint joint) noexcept {
    ...
    return JPH::Vec3(pos[0], pos[1], pos[2]);
}
...
[[nodiscard]] inline JPH::Quat GetBindRotation(Joint joint) noexcept {
    ...
    return JPH::Quat(rot[0], rot[1], rot[2], rot[3]);
}
