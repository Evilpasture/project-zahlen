...
[[nodiscard]] inline float FalloffAt(const WeaponDef& def, float dist) noexcept {
    ...
    return 1.0f + (def.falloffMin - 1.0f) * ((dist - def.falloffStart) / (def.falloffEnd - def.falloffStart));
}

[[nodiscard]] inline float SurfaceCost(float thickness, float hardness) noexcept {
    return std::max(0.05f, thickness) * hardness;
}
...
struct ProceduralWeaponMeshBuilder {
    ...
    void AddBox(JPH::Vec3Arg center, JPH::Vec3Arg size, JPH::Vec4Arg color, JPH::QuatArg rot = JPH::Quat::sIdentity()) {
        ...
        for (const auto& f: faces) {
            ...
            for (const auto& [su, sv]: std::initializer_list<std::pair<float, float>> {{-1.0f, -1.0f}, {1.0f, -1.0f}, {1.0f, 1.0f}, {-1.0f, 1.0f}}) {
                JPH::Vec3 pos = center + rot * (f.n * hs + (f.u * hs) * su + (f.v * hs) * sv);
                positions.push_back({{pos.GetX(), pos.GetY(), pos.GetZ()}});
                attributes.push_back(
                    {.normal  = Math::PackNormal(norm.GetX(), norm.GetY(), norm.GetZ()),
                     .tangent = Math::PackNormal(1.0f, 0.0f, 0.0f),
                     .uv      = Math::PackUV(0.0f, 0.0f),
                     .color   = col}
                );
            }

            indices.insert(indices.end(), {fBase, fBase + 1, fBase + 2, fBase, fBase + 2, fBase + 3});
        }
    }

    Mesh Build(RenderContext& rc) {
        return Mesh {
            .posBuffer   = rc.CreateVertexBuffer(positions.data(), positions.size() * sizeof(VertexPosition)),
            .attrBuffer  = rc.CreateVertexBuffer(attributes.data(), attributes.size() * sizeof(VertexAttributes)),
            .indexBuffer = rc.CreateIndexBuffer(indices.data(), indices.size() * sizeof(uint32_t)),
...
AssetID GetOrGenerateWeaponMeshAsset(RenderContext& rc, WeaponId id) {
    AssetID assetId = HashAssetID(std::format("procedural_weapon_single_mesh_{}", static_cast<uint32_t>(id)));

    if (!rc.GetGPUMesh(assetId).has_value()) {
        ProceduralWeaponMeshBuilder builder;
        JPH::Vec4                   metalCol(0.20f, 0.22f, 0.24f, 1.0f), woodCol(0.40f, 0.25f, 0.12f, 1.0f);
        float                       scale = 0.85f;
        ...
        rc.RegisterGPUMesh(assetId, builder.Build(rc));
    }
    return assetId;
}
...
Entity CreateWeaponModel(Engine* engine, WeaponId id, MaterialID metalMat, MaterialID woodMat) {
    ...
    reg.Add(
        weapon, Components::MeshComponent {
                    .meshAsset     = GetOrGenerateWeaponMeshAsset(engine->GetRenderContext(), id),
                    .materialAsset = (id == WeaponId::Shotgun) ? woodMat : metalMat,
                    .cullRadius    = 2.0f
                }
    );
    return weapon;
